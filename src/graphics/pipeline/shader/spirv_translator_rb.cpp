...
spv::Id SpirvShaderTranslator::PreClampedFloat32To7e3(SpirvBuilder& builder, spv::Id f32_scalar,
  ...
  // Need the source as uint for bit operations.
  {
    ...
    if (!builder.isUintType(source_type)) {
      f32_scalar = builder.createUnaryOp(spv::Op::OpBitcast, type_uint, f32_scalar);
    }
  }
  ...
  spv::Id denormal_biased_f32;
  {
    spv::Instruction* denormal_insert_instruction =
        new spv::Instruction(builder.getUniqueId(), type_uint, spv::Op::OpBitFieldInsert);
  ...
  spv::Id denormal_biased_f32_shift_amount;
  {
    spv::Instruction* denormal_shift_amount_instruction =
        new spv::Instruction(builder.getUniqueId(), type_uint, spv::Op::OpExtInst);
    ...
    denormal_shift_amount_instruction->addIdOperand(
        builder.createBinOp(spv::Op::OpISub, type_uint, builder.makeUintConstant(125),
                            builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, f32_scalar,
  ...
  denormal_biased_f32 = builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint,
                                            denormal_biased_f32, denormal_biased_f32_shift_amount);

  // The normal 7e3 case.
  // Bias the exponent.
  // normal_biased_f32 = f32 - (124 << 23)
  spv::Id normal_biased_f32 = builder.createBinOp(spv::Op::OpISub, type_uint, f32_scalar,
  ...
  spv::Id biased_f32 =
      builder.createTriOp(spv::Op::OpSelect, type_uint,
                          builder.createBinOp(spv::Op::OpULessThan, builder.makeBoolType(), f32_scalar,
  ...
  return builder.createTriOp(
      ...
      builder.createBinOp(
          ...
          builder.createBinOp(spv::Op::OpIAdd, type_uint, biased_f32, builder.makeUintConstant(0x7FFF)),
          builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, biased_f32,
...
spv::Id SpirvShaderTranslator::UnclampedFloat32To7e3(SpirvBuilder& builder, spv::Id f32_scalar,
  ...
  // Need the source as float for clamping.
  {
    ...
    if (!builder.isFloatType(source_type)) {
      f32_scalar = builder.createUnaryOp(spv::Op::OpBitcast, type_float, f32_scalar);
    }
  }

  {
    spv::Instruction* clamp_instruction =
        new spv::Instruction(builder.getUniqueId(), type_float, spv::Op::OpExtInst);
...
spv::Id SpirvShaderTranslator::Float7e3To32(SpirvBuilder& builder, spv::Id f10_uint_scalar,
  ...
  spv::Id f10_unbiased_exponent =
      builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, f10_uint_scalar,
  ...
  spv::Id denormal_mantissa_msb;
  {
    spv::Instruction* denormal_mantissa_msb_instruction =
        new spv::Instruction(builder.getUniqueId(), type_int, spv::Op::OpExtInst);
  ...
  denormal_mantissa_msb = builder.createUnaryOp(spv::Op::OpBitcast, type_uint, denormal_mantissa_msb);
  ...
  spv::Id denormal_f32_unbiased_exponent = builder.createBinOp(
      spv::Op::OpISub, type_uint, denormal_mantissa_msb, builder.makeUintConstant(6));
  // Normalize the mantissa.
  // denormal_f32_mantissa = f10_mantissa << (7 - findMSB(f10_mantissa))
  spv::Id denormal_f32_mantissa =
      builder.createBinOp(spv::Op::OpShiftLeftLogical, type_uint, f10_mantissa,
                          builder.createBinOp(spv::Op::OpISub, type_uint, builder.makeUintConstant(7),
                                              denormal_mantissa_msb));
  // If the 7e3 number is zero, make sure the float32 number is zero too.
  spv::Id f10_mantissa_is_nonzero =
      builder.createBinOp(spv::Op::OpINotEqual, type_bool, f10_mantissa, builder.makeUintConstant(0));
  // Set the unbiased exponent to -124 for zero - 124 will be added later,
  // resulting in zero float32.
  denormal_f32_unbiased_exponent =
      builder.createTriOp(spv::Op::OpSelect, type_uint, f10_mantissa_is_nonzero,
                          denormal_f32_unbiased_exponent, builder.makeUintConstant(uint32_t(-124)));
  denormal_f32_mantissa = builder.createTriOp(spv::Op::OpSelect, type_uint, f10_mantissa_is_nonzero,
                                              denormal_f32_mantissa, builder.makeUintConstant(0));

  // Select the needed conversion depending on whether the number is normal.
  spv::Id f10_is_normal = builder.createBinOp(spv::Op::OpINotEqual, type_bool, f10_unbiased_exponent,
                                              builder.makeUintConstant(0));
  spv::Id f32_unbiased_exponent =
      builder.createTriOp(spv::Op::OpSelect, type_uint, f10_is_normal, f10_unbiased_exponent,
                          denormal_f32_unbiased_exponent);
  spv::Id f32_mantissa = builder.createTriOp(spv::Op::OpSelect, type_uint, f10_is_normal, f10_mantissa,
  ...
  spv::Id f32_shifted;
  {
    ...
    f32_insert_instruction->addIdOperand(builder.createBinOp(
        spv::Op::OpIAdd, type_uint, f32_unbiased_exponent, builder.makeUintConstant(124)));
  ...
  spv::Id f32 = builder.createBinOp(spv::Op::OpShiftLeftLogical, type_uint, f32_shifted,
                                    builder.makeUintConstant(23 - 7));

  if (!result_as_uint) {
    f32 = builder.createUnaryOp(spv::Op::OpBitcast, builder.makeFloatType(32), f32);
  }
...
spv::Id SpirvShaderTranslator::PreClampedDepthTo20e4(SpirvBuilder& builder, spv::Id f32_scalar,
  ...
  spv::Id denormal_biased_f32_shift_amount;
  {
    ...
    denormal_shift_amount_instruction->addImmediateOperand(GLSLstd450UMin);
    denormal_shift_amount_instruction->addIdOperand(
        builder.createBinOp(spv::Op::OpISub, type_uint, builder.makeUintConstant(113 - remap_bias),
                            builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, f32_scalar,
                                                builder.makeUintConstant(23))));
    denormal_shift_amount_instruction->addIdOperand(builder.makeUintConstant(24));
    builder.getBuildPoint()->addInstruction(
  ...
  spv::Id normal_biased_f32 =
      builder.createBinOp(spv::Op::OpISub, type_uint, f32_scalar,
  ...
  if (round_to_nearest_even) {
    ...
    biased_f32 = builder.createBinOp(
        spv::Op::OpIAdd, type_uint,
        builder.createBinOp(spv::Op::OpIAdd, type_uint, biased_f32, builder.makeUintConstant(3)),
        builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, biased_f32,
                            builder.makeUintConstant(3), builder.makeUintConstant(1)));
  }
  return builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, biased_f32,
...
spv::Id SpirvShaderTranslator::Depth20e4To32(SpirvBuilder& builder, spv::Id f24_uint_scalar,
  ...
  spv::Id f24_unbiased_exponent =
      builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, f24_uint_scalar,
  ...
  spv::Id denormal_f32_unbiased_exponent = builder.createBinOp(
      spv::Op::OpISub, type_uint, denormal_mantissa_msb, builder.makeUintConstant(19));
  // Normalize the mantissa.
  // denormal_f32_mantissa = f24_mantissa << (20 - findMSB(f24_mantissa))
  spv::Id denormal_f32_mantissa =
      builder.createBinOp(spv::Op::OpShiftLeftLogical, type_uint, f24_mantissa,
                          builder.createBinOp(spv::Op::OpISub, type_uint, builder.makeUintConstant(20),
                                              denormal_mantissa_msb));
  // If the 20e4 number is zero, make sure the float32 number is zero too.
  spv::Id f24_mantissa_is_nonzero =
      builder.createBinOp(spv::Op::OpINotEqual, type_bool, f24_mantissa, builder.makeUintConstant(0));
  // Set the unbiased exponent to -112 for zero - 112 will be added later,
  // resulting in zero float32.
  denormal_f32_unbiased_exponent = builder.createTriOp(
      spv::Op::OpSelect, type_uint, f24_mantissa_is_nonzero, denormal_f32_unbiased_exponent,
      builder.makeUintConstant(uint32_t(-int32_t(112 - remap_bias))));
  denormal_f32_mantissa = builder.createTriOp(spv::Op::OpSelect, type_uint, f24_mantissa_is_nonzero,
                                              denormal_f32_mantissa, builder.makeUintConstant(0));

  // Select the needed conversion depending on whether the number is normal.
  spv::Id f24_is_normal = builder.createBinOp(spv::Op::OpINotEqual, type_bool, f24_unbiased_exponent,
                                              builder.makeUintConstant(0));
  spv::Id f32_unbiased_exponent =
      builder.createTriOp(spv::Op::OpSelect, type_uint, f24_is_normal, f24_unbiased_exponent,
                          denormal_f32_unbiased_exponent);
  spv::Id f32_mantissa = builder.createTriOp(spv::Op::OpSelect, type_uint, f24_is_normal, f24_mantissa,
                                             denormal_f32_mantissa);
  ...
  spv::Id f32_shifted;
  {
    ...
    f32_insert_instruction->addIdOperand(f32_mantissa);
    f32_insert_instruction->addIdOperand(builder.createBinOp(
        spv::Op::OpIAdd, type_uint, f32_unbiased_exponent, builder.makeUintConstant(112 - remap_bias)));
...
void SpirvShaderTranslator::CompleteFragmentShaderInMain() {
    ...
    if (edram_fragment_shader_interlock_) {
      ...
      if (!features_.demote_to_helper_invocation) {
        ...
        main_fsi_sample_mask_ = builder_->createTriOp(
            spv::Op::OpSelect, type_uint_, builder_->createLoad(var_main_kill_pixel_, spv::NoPrecision),
      ...
      if (!features_.demote_to_helper_invocation) {
        ...
        SpirvBuilder::IfBuilder kill_pixel_if(
            ...
            spv::SelectionControlMask::MaskNone, *builder_);
        // TODO(Triang3l): Use OpTerminateInvocation when SPIR-V 1.6 is
        // targeted.
        builder_->createNoResultOp(spv::Op::OpKill);
  ...
  if ((color_targets_written & 0b1) && !IsExecutionModeEarlyFragmentTests()) {
    ...
    if (!edram_fragment_shader_interlock_ && output_fragment_sample_mask_ != spv::NoResult) {
      ...
      builder_->createStore(
          ...
          builder_->createAccessChain(spv::StorageClass::Output, output_fragment_sample_mask_,
                                      id_vector_temp_));
    }
    ...
    spv::Id alpha_test_function = builder_->createTriOp(
        spv::Op::OpBitFieldUExtract, type_uint_, main_system_constant_flags_,
    ...
    SpirvBuilder::IfBuilder if_alpha_test_function_is_non_always(
        builder_->createBinOp(
            spv::Op::OpINotEqual, type_bool_, alpha_test_function,
        ...
        spv::SelectionControlMask::DontFlatten, *builder_);
    {
      ...
      spv::Id alpha_test_alpha = builder_->createLoad(
          builder_->createAccessChain(edram_fragment_shader_interlock_ ? spv::StorageClass::Function
      ...
      SpirvBuilder::IfBuilder if_alpha_test_function_is_not_equal(
          builder_->createBinOp(
              spv::Op::OpIEqual, type_bool_, alpha_test_function,
              builder_->makeUintConstant(uint32_t(xenos::CompareFunction::kNotEqual))),
          spv::SelectionControlMask::DontFlatten, *builder_, 1, 2);
      spv::Id alpha_test_result_not_equal;
      {
        // "Not equal" function.
        alpha_test_result_not_equal = builder_->createBinOp(spv::Op::OpFUnordNotEqual, type_bool_,
      ...
      spv::Id alpha_test_result_non_not_equal;
      {
        ...
        static const spv::Op kAlphaTestOps[] = {spv::Op::OpFOrdLessThan, spv::Op::OpFOrdEqual,
                                                spv::Op::OpFOrdGreaterThan};
        for (uint32_t i = 0; i < 3; ++i) {
          spv::Id alpha_test_comparison_result = builder_->createBinOp(
              spv::Op::OpLogicalAnd, type_bool_,
              builder_->createBinOp(kAlphaTestOps[i], type_bool_, alpha_test_alpha,
                                    alpha_test_reference),
              builder_->createBinOp(
                  spv::Op::OpINotEqual, type_bool_,
                  builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, alpha_test_function,
                                        builder_->makeUintConstant(UINT32_C(1) << i)),
                  const_uint_0_));
          if (i) {
            alpha_test_result_non_not_equal =
                builder_->createBinOp(spv::Op::OpLogicalOr, type_bool_, alpha_test_result_non_not_equal,
      ...
      if (edram_fragment_shader_interlock_ && !features_.demote_to_helper_invocation) {
        ...
        fsi_sample_mask_in_rt_0_alpha_tests =
            builder_->createTriOp(spv::Op::OpSelect, type_uint_, alpha_test_result,
                                  fsi_sample_mask_in_rt_0_alpha_tests, const_uint_0_);
      } else {
        SpirvBuilder::IfBuilder alpha_test_kill_if(
            builder_->createUnaryOp(spv::Op::OpLogicalNot, type_bool_, alpha_test_result),
            spv::SelectionControlMask::DontFlatten, *builder_);
        ...
        if (edram_fragment_shader_interlock_) {
          ...
          builder_->createNoResultOp(spv::Op::OpDemoteToHelperInvocationEXT);
        } else {
          // TODO(Triang3l): Use OpTerminateInvocation when SPIR-V 1.6 is
          // targeted.
          builder_->createNoResultOp(spv::Op::OpKill);
    ...
    if (edram_fragment_shader_interlock_ || output_fragment_sample_mask_ != spv::NoResult) {
      ...
      SpirvBuilder::IfBuilder if_alpha_to_coverage_enabled(
          builder_->createBinOp(spv::Op::OpINotEqual, type_bool_, alpha_to_mask, const_uint_0_),
          spv::SelectionControlMask::DontFlatten, *builder_);
      {
        if (edram_fragment_shader_interlock_) {
          fsi_pixel_potentially_killed = true;
        ...
        spv::Id alpha_to_coverage_offset = builder_->createTriOp(
            spv::Op::OpBitFieldUExtract, type_uint_, alpha_to_mask,
            builder_->createBinOp(
        ...
        spv::Id alpha_to_coverage_offset_float =
            builder_->createUnaryOp(spv::Op::OpConvertUToF, type_float_, alpha_to_coverage_offset);
        if (msaa_samples == spv::NoResult) {
          msaa_samples = LoadMsaaSamplesFromFlags();
        }
        auto alpha_to_coverage_apply_sample = [&](spv::Id sample_mask, uint32_t guest_sample_index,
          ...
          spv::Id sample_threshold = builder_->createBinOp(
              spv::Op::OpFSub, type_float_, builder_->makeFloatConstant(threshold_base),
              builder_->createBinOp(spv::Op::OpFMul, type_float_, alpha_to_coverage_offset_float,
                                    builder_->makeFloatConstant(threshold_offset_scale)));
          spv::Id sample_covered = builder_->createBinOp(spv::Op::OpFOrdGreaterThanEqual, type_bool_,
                                                         alpha_to_coverage_alpha, sample_threshold);
          if (edram_fragment_shader_interlock_) {
            return builder_->createTriOp(
                spv::Op::OpSelect, type_uint_, sample_covered, sample_mask,
                builder_->createBinOp(
                    spv::Op::OpBitwiseAnd, type_uint_, sample_mask,
                    builder_->makeUintConstant(~((UINT32_C(1) << guest_sample_index) |
                                                 (UINT32_C(1) << (guest_sample_index + 4))))));
          }
          return builder_->createTriOp(
              spv::Op::OpSelect, type_uint_, sample_covered,
              builder_->createBinOp(spv::Op::OpBitwiseOr, type_uint_, sample_mask,
        ...
        builder_->createBranch(false, &block_alpha_to_coverage_msaa_merge);
        // 2x.
        ...
        alpha_to_coverage_mask = builder_->createOp(spv::Op::OpPhi, type_uint_, id_vector_temp_);
        if (edram_fragment_shader_interlock_) {
          fsi_sample_mask_in_rt_0_alpha_tests = alpha_to_coverage_mask;
        } else {
          SpirvBuilder::IfBuilder alpha_to_coverage_kill_if(
              builder_->createBinOp(spv::Op::OpIEqual, type_bool_, alpha_to_coverage_mask,
                                    const_uint_0_),
              spv::SelectionControlMask::DontFlatten, *builder_);
          // TODO(Triang3l): Use OpTerminateInvocation when SPIR-V 1.6 is
          // targeted.
          ...
          id_vector_temp_.push_back(const_int_0_);
          builder_->createStore(
              builder_->createUnaryOp(spv::Op::OpBitcast, type_int_, alpha_to_coverage_mask),
              builder_->createAccessChain(spv::StorageClass::Output, output_fragment_sample_mask_,
                                          id_vector_temp_));
        }
      }
    ...
    if (edram_fragment_shader_interlock_) {
      ...
      builder_->createBranch(false, block_fsi_rt_0_alpha_tests_rt_written_merge);
      ...
      if (!features_.demote_to_helper_invocation) {
        ...
        main_fsi_sample_mask_ = builder_->createOp(spv::Op::OpPhi, type_uint_, id_vector_temp_);
      }
    }
  }
    ...
    if (fsi_pixel_potentially_killed) {
      if (features_.demote_to_helper_invocation) {
        ...
        main_fsi_sample_mask_ = builder_->createTriOp(
            ...
            builder_->createOp(spv::Op::OpIsHelperInvocationEXT, type_bool_, id_vector_temp_),
            const_uint_0_, main_fsi_sample_mask_);
      }
    ...
    if (FSI_IsDepthStencilEarly()) {
      ...
      for (uint32_t i = 0; i < 4; ++i) {
        spv::Id sample_late_depth_stencil_write_needed = builder_->createBinOp(
            spv::Op::OpINotEqual, type_bool_,
            builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, main_fsi_sample_mask_,
                                  builder_->makeUintConstant(uint32_t(1) << (4 + i))),
            const_uint_0_);
        SpirvBuilder::IfBuilder if_sample_late_depth_stencil_write_needed(
            sample_late_depth_stencil_write_needed, spv::SelectionControlMask::DontFlatten,
            *builder_);
      ...
      if (color_targets_written) {
        ...
        // depth / stencil test.
        color_write_depth_stencil_condition = builder_->createBinOp(
            spv::Op::OpINotEqual, type_bool_, main_fsi_sample_mask_, const_uint_0_);
      }
    }
  ...
  // ROV ZPD counter: count MSAA samples that passed depth/stencil.
  if (edram_fragment_shader_interlock_) {
    spv::Id const_scope_device = builder_->makeUintConstant(
        static_cast<unsigned int>(spv::Scope::Device));
    spv::Id zpd_counter_index = builder_->createLoad(
        builder_->createAccessChain(spv::StorageClass::Uniform,
                                    uniform_system_constants_,
                                    {builder_->makeIntConstant(kSystemConstantZpdRovCounterIndex)}),
        spv::NoPrecision);
    spv::Id zpd_active = builder_->createBinOp(
        spv::Op::OpINotEqual, type_bool_, zpd_counter_index,
        builder_->makeUintConstant(UINT32_MAX));
    SpirvBuilder::IfBuilder if_zpd_active(zpd_active, spv::SelectionControlMask::DontFlatten, *builder_);
    {
      spv::Id coverage = builder_->createBinOp(
          spv::Op::OpBitwiseAnd, type_uint_, main_fsi_sample_mask_,
          builder_->makeUintConstant((uint32_t(1) << 4) - 1));
      spv::Id sample_count = builder_->createUnaryOp(
          spv::Op::OpBitCount, type_uint_, coverage);
      spv::Id any_passed = builder_->createBinOp(
          spv::Op::OpINotEqual, type_bool_, sample_count, const_uint_0_);
      SpirvBuilder::IfBuilder if_any_passed(any_passed, spv::SelectionControlMask::DontFlatten, *builder_);
      {
        spv::StorageClass storage_class = features_.spirv_version >= spv::Spv_1_3
            ? spv::StorageClass::StorageBuffer : spv::StorageClass::Uniform;
        id_vector_temp_.clear();
        id_vector_temp_.push_back(const_int_0_);
        id_vector_temp_.push_back(zpd_counter_index);
        spv::Id counter_ptr = builder_->createAccessChain(
            storage_class, buffer_zpd_counter_, id_vector_temp_);
        builder_->createQuadOp(spv::Op::OpAtomicIAdd, type_uint_, counter_ptr,
                               const_scope_device, const_uint_0_, sample_count);
      }
      if_any_passed.makeEndIf();
    }
    if_zpd_active.makeEndIf();
  }
  ...
  if (color_targets_written) {
    ...
    while (rex::bit_scan_forward(color_targets_remaining, &color_target_index)) {
      ...
      color = builder_->createNoContractionBinOp(
          spv::Op::OpVectorTimesScalar, type_float4_, color,
          builder_->createLoad(
              builder_->createAccessChain(spv::StorageClass::Uniform, uniform_system_constants_,
                                          id_vector_temp_),
              spv::NoPrecision));
      ...
      if (edram_fragment_shader_interlock_) {
        ...
        spv::Id fsi_color_written = builder_->createBinOp(
            spv::Op::OpINotEqual, type_bool_,
            builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, fsi_color_targets_written,
        ...
        SpirvBuilder::IfBuilder if_fsi_color_written(
            fsi_color_written, spv::SelectionControlMask::DontFlatten, *builder_, 2, 1);
        ...
        spv::Id rt_write_mask_not_empty = builder_->createBinOp(
            ...
            builder_->createBinOp(spv::Op::OpINotEqual, type_bool_, rt_keep_mask[0], const_uint32_max),
            builder_->createBinOp(spv::Op::OpINotEqual, type_bool_, rt_keep_mask[1], const_uint32_max));
        SpirvBuilder::IfBuilder if_rt_write_mask_not_empty(
            rt_write_mask_not_empty, spv::SelectionControlMask::DontFlatten, *builder_);
        ...
        spv::Id rt_is_64bpp = builder_->createBinOp(
            spv::Op::OpINotEqual, type_bool_,
            builder_->createBinOp(
                spv::Op::OpBitwiseAnd, type_uint_, rt_format_with_flags,
            ...
            builder_->createBinOp(
                ...
                builder_->createBinOp(
                    ...
                    builder_->createLoad(
                        ...
                        spv::NoPrecision),
                    builder_->createTriOp(spv::Op::OpSelect, type_uint_, rt_is_64bpp,
        ...
        spv::Id rt_blend_enabled =
            builder_->createBinOp(spv::Op::OpINotEqual, type_bool_, rt_blend_factors_equations,
                                  builder_->makeUintConstant(0x00010001));
        SpirvBuilder::IfBuilder if_rt_blend_enabled(
            rt_blend_enabled, spv::SelectionControlMask::DontFlatten, *builder_);
        {
          // Blending path.
          ...
          spv::Id rt_color_is_fixed_point = builder_->createBinOp(
              spv::Op::OpINotEqual, type_bool_,
              builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, rt_format_with_flags,
          ...
          spv::Id rt_color_source_factor =
              builder_->createTriOp(spv::Op::OpBitFieldUExtract, type_uint_, rt_blend_factors_equations,
          ...
          for (uint32_t i = 0; i < 2; ++i) {
            rt_replace_mask[i] = builder_->createUnaryOp(spv::Op::OpNot, type_uint_, rt_keep_mask[i]);
          }

          // Blend and mask each sample.
          for (uint32_t i = 0; i < 4; ++i) {
            SpirvBuilder::IfBuilder if_sample_covered(
                fsi_samples_covered[i], spv::SelectionControlMask::DontFlatten, *builder_);
            ...
            spv::Id rt_access_chain_0 = builder_->createAccessChain(
                features_.spirv_version >= spv::Spv_1_3 ? spv::StorageClass::StorageBuffer
                                                        : spv::StorageClass::Uniform,
                buffer_edram_, id_vector_temp_);
            id_vector_temp_.back() =
                builder_->createBinOp(spv::Op::OpIAdd, type_int_, rt_sample_address, fsi_const_int_1);
            ...
            {
              SpirvBuilder::IfBuilder if_64bpp(rt_is_64bpp, spv::SelectionControlMask::DontFlatten,
                                               *builder_);
            ...
            builder_->createStore(
                builder_->createBinOp(spv::Op::OpBitwiseOr, type_uint_,
                                      builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_,
                                             ...
                                             *builder_);
            {
              builder_->createStore(
                  builder_->createBinOp(
                      spv::Op::OpBitwiseOr, type_uint_,
                      builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, dest_packed[1],
                                            rt_keep_mask[1]),
                      builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, result_packed[1],
                                            rt_replace_mask[1])),
                  rt_access_chain_1);
            }
            if_64bpp.makeEndIf();
        ...
        if_rt_blend_enabled.makeBeginElse();
        {
          ...
          spv::Id rt_keep_mask_not_empty = builder_->createBinOp(
              spv::Op::OpLogicalOr, type_bool_,
              builder_->createBinOp(spv::Op::OpINotEqual, type_bool_, rt_keep_mask[0], const_uint_0_),
              builder_->createBinOp(spv::Op::OpINotEqual, type_bool_, rt_keep_mask[1], const_uint_0_));

          SpirvBuilder::IfBuilder if_rt_keep_mask_not_empty(
              rt_keep_mask_not_empty, spv::SelectionControlMask::DontFlatten, *builder_);
          {
            // Loading and masking path.
            std::array<spv::Id, 2> color_packed_masked;
            for (uint32_t i = 0; i < 2; ++i) {
              color_packed_masked[i] = builder_->createBinOp(
                  spv::Op::OpBitwiseAnd, type_uint_, color_packed[i],
                  builder_->createUnaryOp(spv::Op::OpNot, type_uint_, rt_keep_mask[i]));
            }
            for (uint32_t i = 0; i < 4; ++i) {
              SpirvBuilder::IfBuilder if_sample_covered(
                  fsi_samples_covered[i], spv::SelectionControlMask::DontFlatten, *builder_);
              ...
              SpirvBuilder::IfBuilder if_64bpp(rt_is_64bpp, spv::SelectionControlMask::DontFlatten,
                                               *builder_);
              {
                id_vector_temp_.back() = builder_->createBinOp(spv::Op::OpIAdd, type_int_,
            ...
            for (uint32_t i = 0; i < 4; ++i) {
                                               ...
                                               *builder_);
              {
                id_vector_temp_.back() = builder_->createBinOp(
                    spv::Op::OpIAdd, type_int_, id_vector_temp_.back(), fsi_const_int_1);
        ...
        // Xbox 360 applies them).
        if (color_target_index == 0) {
          Modification premult_mod = GetSpirvShaderModification();
          xenos::BlendFactor rgb_factor =
              premult_mod.pixel.rt0_blend_rgb_factor_for_premult;
          xenos::BlendFactor a_factor =
              premult_mod.pixel.rt0_blend_a_factor_for_premult;
          bool premult_rgb = rgb_factor != xenos::BlendFactor::kOne;
          bool premult_a = a_factor != xenos::BlendFactor::kOne;
          if (premult_rgb || premult_a) {
            ...
            spv::Id color_rgb = builder_->createRvalueSwizzle(
                spv::NoPrecision, type_float3_, color, uint_vector_temp_);
            spv::Id color_a = builder_->createCompositeExtract(color, type_float_, 3);
            if (premult_rgb) {
              switch (rgb_factor) {
                case xenos::BlendFactor::kZero:
                  color_rgb = const_float3_0_;
                  break;
                case xenos::BlendFactor::kSrcColor:
                  color_rgb = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float3_, color_rgb, color_rgb);
                  break;
                case xenos::BlendFactor::kOneMinusSrcColor: {
                  spv::Id one_minus = builder_->createNoContractionBinOp(
                      spv::Op::OpFSub, type_float3_, const_float3_1_, color_rgb);
                  color_rgb = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float3_, color_rgb, one_minus);
                  break;
                }
                case xenos::BlendFactor::kSrcAlpha: {
                  ...
                  uint_vector_temp_.push_back(3);
                  uint_vector_temp_.push_back(3);
                  spv::Id alpha_rgb = builder_->createRvalueSwizzle(
                      spv::NoPrecision, type_float3_, color, uint_vector_temp_);
                  color_rgb = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float3_, color_rgb, alpha_rgb);
                  break;
                }
                case xenos::BlendFactor::kOneMinusSrcAlpha: {
                  spv::Id one_minus = builder_->createNoContractionBinOp(
                      spv::Op::OpFSub, type_float_, const_float_1_, color_a);
                  uint_vector_temp_.clear();
                  uint_vector_temp_.push_back(0);
                  uint_vector_temp_.push_back(0);
                  uint_vector_temp_.push_back(0);
                  spv::Id one_minus_xxx = builder_->createRvalueSwizzle(
                      spv::NoPrecision, type_float3_, one_minus, uint_vector_temp_);
                  color_rgb = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float3_, color_rgb, one_minus_xxx);
                  break;
                }
                ...
                case xenos::BlendFactor::kConstantAlpha: {
                  ...
                  spv::Id blend_constant = builder_->createLoad(
                      ...
                      spv::NoPrecision);
                  spv::Id factor;
                  if (rgb_factor == xenos::BlendFactor::kConstantColor) {
                    uint_vector_temp_.clear();
                    uint_vector_temp_.push_back(0);
                    uint_vector_temp_.push_back(1);
                    uint_vector_temp_.push_back(2);
                    factor = builder_->createRvalueSwizzle(
                        spv::NoPrecision, type_float3_, blend_constant,
                        uint_vector_temp_);
                  } else {
                    spv::Id blend_alpha = builder_->createCompositeExtract(
                        blend_constant, type_float_, 3);
                    id_vector_temp_.clear();
                    id_vector_temp_.push_back(blend_alpha);
                    id_vector_temp_.push_back(blend_alpha);
                    id_vector_temp_.push_back(blend_alpha);
                    factor = builder_->createCompositeConstruct(
                        type_float3_, id_vector_temp_);
                  }
                  color_rgb = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float3_, color_rgb, factor);
                  break;
                }
                ...
                case xenos::BlendFactor::kOneMinusConstantAlpha: {
                  ...
                  spv::Id blend_constant = builder_->createLoad(
                      ...
                      spv::NoPrecision);
                  spv::Id factor;
                  if (rgb_factor == xenos::BlendFactor::kOneMinusConstantColor) {
                    uint_vector_temp_.clear();
                    uint_vector_temp_.push_back(0);
                    uint_vector_temp_.push_back(1);
                    uint_vector_temp_.push_back(2);
                  ...
                  spv::Id one_minus = builder_->createNoContractionBinOp(
                      spv::Op::OpFSub, type_float3_, const_float3_1_, factor);
                  color_rgb = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float3_, color_rgb, one_minus);
                  break;
                }
            ...
            if (premult_a) {
              switch (a_factor) {
                case xenos::BlendFactor::kZero:
                  color_a = const_float_0_;
                  break;
                case xenos::BlendFactor::kSrcColor:
                case xenos::BlendFactor::kSrcAlpha:
                  color_a = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float_, color_a, color_a);
                  break;
                case xenos::BlendFactor::kOneMinusSrcColor:
                case xenos::BlendFactor::kOneMinusSrcAlpha: {
                  spv::Id one_minus = builder_->createNoContractionBinOp(
                      spv::Op::OpFSub, type_float_, const_float_1_, color_a);
                  color_a = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float_, color_a, one_minus);
                  break;
                }
                case xenos::BlendFactor::kConstantColor:
                case xenos::BlendFactor::kConstantAlpha: {
                  ...
                  spv::Id blend_alpha = builder_->createCompositeExtract(
                      blend_constant, type_float_, 3);
                  color_a = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float_, color_a, blend_alpha);
                  break;
                }
                case xenos::BlendFactor::kOneMinusConstantColor:
                case xenos::BlendFactor::kOneMinusConstantAlpha: {
                  ...
                  spv::Id blend_alpha = builder_->createCompositeExtract(
                      blend_constant, type_float_, 3);
                  spv::Id one_minus = builder_->createNoContractionBinOp(
                      spv::Op::OpFSub, type_float_, const_float_1_, blend_alpha);
                  color_a = builder_->createNoContractionBinOp(
                      spv::Op::OpFMul, type_float_, color_a, one_minus);
                  break;
                }
            ...
            id_vector_temp_.push_back(color_a);
        ...
        spv::Id is_gamma = builder_->createBinOp(
            ...
            const_uint_0_);
        SpirvBuilder::IfBuilder if_gamma(is_gamma, spv::SelectionControlMask::DontFlatten, *builder_);
    ...
    if (depth_stencil_mode == Modification::DepthStencilMode::kFloat24Truncating ||
      ...
      if (current_shader().writes_depth()) {
        ...
        depth = builder_->createTriBuiltinCall(
            ...
            builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_, depth,
  ...
  if (edram_fragment_shader_interlock_) {
    if (block_fsi_if_after_depth_stencil_merge) {
      builder_->createBranch(false, block_fsi_if_after_depth_stencil_merge);
      builder_->setBuildPoint(block_fsi_if_after_depth_stencil_merge);
    }

    if (block_fsi_if_after_kill_merge) {
      builder_->createBranch(false, block_fsi_if_after_kill_merge);
      builder_->setBuildPoint(block_fsi_if_after_kill_merge);
    }

    if (FSI_IsDepthStencilEarly()) {
      builder_->createBranch(false, main_fsi_early_depth_stencil_execute_quad_merge_);
      builder_->setBuildPoint(main_fsi_early_depth_stencil_execute_quad_merge_);
    }

    builder_->createNoResultOp(spv::Op::OpEndInvocationInterlockEXT);
  }
}

spv::Id SpirvShaderTranslator::LoadMsaaSamplesFromFlags() {
  return builder_->createTriOp(spv::Op::OpBitFieldUExtract, type_uint_, main_system_constant_flags_,
...
void SpirvShaderTranslator::FSI_LoadSampleMask(spv::Id msaa_samples) {
  ...
  spv::Id input_sample_mask_value = builder_->createUnaryOp(
      ...
      builder_->createLoad(
          builder_->createAccessChain(spv::StorageClass::Input, input_sample_mask_, id_vector_temp_),
          spv::NoPrecision));
  ...
  builder_->createSelectionMerge(&block_msaa_merge, spv::SelectionControlMask::DontFlatten);
  {
  ...
  builder_->createBranch(false, &block_msaa_merge);
  ...
  if (native_2x_msaa_no_attachments_) {
    ...
    sample_mask_2x = builder_->createBinOp(
        ...
        builder_->createUnaryOp(spv::Op::OpBitReverse, type_uint_, input_sample_mask_value),
        builder_->makeUintConstant(32 - 2));
  } else {
    // 0 and 3 to 0 and 1.
    sample_mask_2x = builder_->createQuadOp(
        spv::Op::OpBitFieldInsert, type_uint_, input_sample_mask_value,
        builder_->createTriOp(spv::Op::OpBitFieldUExtract, type_uint_, input_sample_mask_value,
...
void SpirvShaderTranslator::FSI_LoadEdramOffsets(spv::Id msaa_samples) {
        ...
        builder_->createUnaryOp(
            ...
            builder_->createLoad(
                builder_->createAccessChain(spv::StorageClass::Input, input_fragment_coordinates_,
                                            id_vector_temp_),
                spv::NoPrecision)),
        builder_->createTriOp(spv::Op::OpSelect, type_uint_, axis_has_two_msaa_samples, const_uint_1,
                              const_uint_0_));
  }
  ...
  for (uint32_t i = 0; i < 2; ++i) {
    ...
    tile_half_index[i] =
        builder_->createBinOp(spv::Op::OpUDiv, type_uint_, sample_x_or_y, tile_half_width_or_height);
    tile_half_sample_coordinates[i] =
        builder_->createBinOp(spv::Op::OpUMod, type_uint_, sample_x_or_y, tile_half_width_or_height);
  }
  ...
  spv::Id row_offset_in_tile_at_32bpp = builder_->createBinOp(
      spv::Op::OpIMul, type_uint_, tile_half_sample_coordinates[1], const_tile_width);
  ...
  spv::Id offset_in_first_tile_half_at_32bpp = builder_->createBinOp(
      spv::Op::OpIAdd, type_uint_,
      builder_->createBinOp(
          spv::Op::OpIAdd, type_uint_, tile_row_offset_at_32bpp,
          builder_->createBinOp(
              spv::Op::OpIAdd, type_uint_,
              builder_->createBinOp(spv::Op::OpIMul, type_uint_, const_tile_size,
  ...
  spv::Id is_second_tile_half = builder_->createBinOp(
      spv::Op::OpINotEqual, type_bool_,
      builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, tile_half_index[0], const_uint_1),
      const_uint_0_);
  ...
  main_fsi_address_depth_ = builder_->createBinOp(
      ...
      builder_->createBinOp(
          ...
          builder_->createLoad(
              ...
              spv::NoPrecision),
          builder_->createBinOp(
              spv::Op::OpIAdd, type_uint_, offset_in_first_tile_half_at_32bpp,
              builder_->createTriOp(spv::Op::OpSelect, type_uint_, is_second_tile_half, const_uint_0_,
  ...
  if (current_shader().writes_color_targets()) {
    ...
    main_fsi_offset_32bpp_ =
        builder_->createBinOp(spv::Op::OpIAdd, type_uint_, offset_in_first_tile_half_at_32bpp,
                              builder_->createTriOp(spv::Op::OpSelect, type_uint_, is_second_tile_half,
    ...
    main_fsi_offset_64bpp_ = builder_->createBinOp(
        ...
        builder_->createBinOp(
            spv::Op::OpIAdd, type_uint_,
            builder_->createBinOp(spv::Op::OpShiftLeftLogical, type_uint_, tile_row_offset_at_32bpp,
                                  const_uint_1),
            builder_->createBinOp(
                spv::Op::OpIAdd, type_uint_,
                builder_->createBinOp(spv::Op::OpIMul, type_uint_, const_tile_size, tile_half_index[0]),
                row_offset_in_tile_at_32bpp)),
        builder_->createBinOp(spv::Op::OpShiftLeftLogical, type_uint_, tile_half_sample_coordinates[0],
                              const_uint_1));
  }
}
...
spv::Id SpirvShaderTranslator::FSI_AddSampleOffset(spv::Id sample_0_address, uint32_t sample_index,
  ...
  if (sample_index == 1) {
    ...
    if (is_64bpp != spv::NoResult) {
      sample_offset = builder_->createTriOp(
          spv::Op::OpSelect, type_int_, is_64bpp,
  ...
  return builder_->createBinOp(spv::Op::OpIAdd, type_int_, sample_0_address, sample_offset);
}
...
void SpirvShaderTranslator::FSI_DepthStencilTest(spv::Id msaa_samples,
  ...
  if (shader_writes_depth) {
    ...
    depth_dxy[0] = builder_->createUnaryOp(spv::Op::OpDPdxCoarse, type_float_, center_depth32_unbiased);
    depth_dxy[1] = builder_->createUnaryOp(spv::Op::OpDPdyCoarse, type_float_, center_depth32_unbiased);
  }
  ...
  spv::Id not_early_write =
      (is_early && implicit_early_z_write_allowed)
          ? builder_->createUnaryOp(spv::Op::OpLogicalNot, type_bool_, early_write)
          : spv::NoResult;
  ...
  spv::Id poly_offset_scale, poly_offset_offset, stencil_parameters;
  {
    ...
    spv::Id poly_offset_back_scale = builder_->createLoad(
        builder_->createAccessChain(spv::StorageClass::Uniform, uniform_system_constants_,
                                    id_vector_temp_),
        spv::NoPrecision);
    poly_offset_scale = builder_->createTriOp(spv::Op::OpSelect, type_float_, front_facing,
    ...
    spv::Id poly_offset_back_offset = builder_->createLoad(
        builder_->createAccessChain(spv::StorageClass::Uniform, uniform_system_constants_,
                                    id_vector_temp_),
        spv::NoPrecision);
    poly_offset_offset = builder_->createTriOp(spv::Op::OpSelect, type_float_, front_facing,
    ...
    spv::Id stencil_parameters_back = builder_->createLoad(
        ...
        spv::NoPrecision);
    stencil_parameters =
        builder_->createTriOp(spv::Op::OpSelect, type_uint2_,
  ...
  spv::Id stencil_reference = builder_->createTriOp(
      spv::Op::OpBitFieldUExtract, type_uint_, stencil_reference_masks, const_uint_0_, const_uint_8);
  spv::Id stencil_read_mask = builder_->createTriOp(
      spv::Op::OpBitFieldUExtract, type_uint_, stencil_reference_masks, const_uint_8, const_uint_8);
  spv::Id stencil_reference_read_masked =
      builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, stencil_reference, stencil_read_mask);
  spv::Id stencil_write_mask =
      builder_->createTriOp(spv::Op::OpBitFieldUExtract, type_uint_, stencil_reference_masks,
                            builder_->makeUintConstant(16), const_uint_8);
  spv::Id stencil_write_keep_mask =
      builder_->createUnaryOp(spv::Op::OpNot, type_uint_, stencil_write_mask);
  spv::Id stencil_func_ops = builder_->createCompositeExtract(stencil_parameters, type_uint_, 1);
  spv::Id stencil_pass_if_less =
      builder_->createBinOp(spv::Op::OpINotEqual, type_bool_,
                            builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, stencil_func_ops,
  ...
  if (!shader_writes_depth) {
    ...
    spv::Id slope_scaled_poly_offset = builder_->createNoContractionBinOp(
        spv::Op::OpFMul, type_float_, poly_offset_scale, depth_max_slope);
    spv::Id poly_offset = builder_->createNoContractionBinOp(
        spv::Op::OpFAdd, type_float_, slope_scaled_poly_offset, poly_offset_offset);
    ...
    center_depth32_biased = builder_->createNoContractionBinOp(
        spv::Op::OpFAdd, type_float_, center_depth32_unbiased, poly_offset);
  }
  ...
  for (uint32_t i = 0; i < 4; ++i) {
    spv::Id sample_covered =
        builder_->createBinOp(spv::Op::OpINotEqual, type_bool_,
                              builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, new_sample_mask,
                                                    builder_->makeUintConstant(uint32_t(1) << i)),
                              const_uint_0_);
    SpirvBuilder::IfBuilder if_sample_covered(sample_covered, spv::SelectionControlMask::DontFlatten,
                                              *builder_);
    ...
    if (shader_writes_depth) {
      ...
      switch (i) {
        case 0: {
          ...
          if (native_2x_msaa_no_attachments_) {
            ...
            for (uint32_t j = 0; j < 2; ++j) {
              sample_location[j] = builder_->createTriOp(
                  spv::Op::OpSelect, type_float_, msaa_is_4x, sample_location[j],
          ...
          // 1x case.
          for (uint32_t j = 0; j < 2; ++j) {
            sample_location[j] = builder_->createTriOp(spv::Op::OpSelect, type_float_, msaa_is_2x_4x,
        ...
        case 1: {
          ...
          for (uint32_t j = 0; j < 2; ++j) {
            sample_location[j] = builder_->createTriOp(
                spv::Op::OpSelect, type_float_, msaa_is_4x,
      ...
      for (uint32_t j = 0; j < 2; ++j) {
        sample_depth_dxy[j] = builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_,
                                                                 sample_location[j], depth_dxy[j]);
      }
      sample_depth32 = builder_->createTriBuiltinCall(
          type_float_, ext_inst_glsl_std_450_, GLSLstd450NClamp,
          builder_->createNoContractionBinOp(
              spv::Op::OpFAdd, type_float_, center_depth32_biased,
              builder_->createNoContractionBinOp(spv::Op::OpFAdd, type_float_, sample_depth_dxy[0],
                                                 sample_depth_dxy[1])),
          const_float_0_, const_float_1_);
    }

    // Convert the new depth to 24-bit.
    SpirvBuilder::IfBuilder depth_format_if(depth_is_float24, spv::SelectionControlMask::DontFlatten,
                                            *builder_);
    ...
    spv::Id sample_depth_unorm24 = builder_->createUnaryOp(
        ...
        builder_->createUnaryBuiltinCall(
            ...
            builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_, sample_depth32,
    ...
    spv::Id old_depth = builder_->createBinOp(spv::Op::OpShiftRightLogical, type_uint_,
                                              old_depth_stencil, const_uint_8);
    spv::Id depth_passed = builder_->createBinOp(
        spv::Op::OpLogicalAnd, type_bool_, depth_pass_if_less,
        builder_->createBinOp(spv::Op::OpULessThan, type_bool_, sample_depth24, old_depth));
    depth_passed = builder_->createBinOp(
        spv::Op::OpLogicalOr, type_bool_, depth_passed,
        builder_->createBinOp(
            spv::Op::OpLogicalAnd, type_bool_, depth_pass_if_equal,
            builder_->createBinOp(spv::Op::OpIEqual, type_bool_, sample_depth24, old_depth)));
    depth_passed = builder_->createBinOp(
        spv::Op::OpLogicalOr, type_bool_, depth_passed,
        builder_->createBinOp(
            spv::Op::OpLogicalAnd, type_bool_, depth_pass_if_greater,
            builder_->createBinOp(spv::Op::OpUGreaterThan, type_bool_, sample_depth24, old_depth)));

    // Perform the stencil test if enabled.
    SpirvBuilder::IfBuilder stencil_if(stencil_enabled, spv::SelectionControlMask::DontFlatten,
                                       *builder_);
    ...
    spv::Id new_stencil_and_old_depth_if_stencil_enabled;
    {
      ...
      spv::Id old_stencil_read_masked = builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_,
                                                              old_depth_stencil, stencil_read_mask);
      stencil_passed_if_enabled = builder_->createBinOp(
          spv::Op::OpLogicalAnd, type_bool_, stencil_pass_if_less,
          builder_->createBinOp(spv::Op::OpULessThan, type_bool_, stencil_reference_read_masked,
                                old_stencil_read_masked));
      stencil_passed_if_enabled = builder_->createBinOp(
          spv::Op::OpLogicalOr, type_bool_, stencil_passed_if_enabled,
          builder_->createBinOp(
              spv::Op::OpLogicalAnd, type_bool_, stencil_pass_if_equal,
              builder_->createBinOp(spv::Op::OpIEqual, type_bool_, stencil_reference_read_masked,
                                    old_stencil_read_masked)));
      stencil_passed_if_enabled = builder_->createBinOp(
          spv::Op::OpLogicalOr, type_bool_, stencil_passed_if_enabled,
          builder_->createBinOp(
              spv::Op::OpLogicalAnd, type_bool_, stencil_pass_if_greater,
              builder_->createBinOp(spv::Op::OpUGreaterThan, type_bool_, stencil_reference_read_masked,
                                    old_stencil_read_masked)));
      spv::Id stencil_op = builder_->createTriOp(
          spv::Op::OpBitFieldUExtract, type_uint_, stencil_func_ops,
          builder_->createTriOp(
              spv::Op::OpSelect, type_uint_, stencil_passed_if_enabled,
              builder_->createTriOp(spv::Op::OpSelect, type_uint_, depth_passed,
      ...
      builder_->createSelectionMerge(&block_stencil_op_merge, spv::SelectionControlMask::DontFlatten);
      {
      ...
      builder_->createBranch(false, &block_stencil_op_merge);
      ...
      spv::Id new_stencil_in_low_bits_increment_clamp = builder_->createBinOp(
          ...
          builder_->createBinBuiltinCall(
              ...
              builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, old_depth_stencil,
      ...
      spv::Id new_stencil_in_low_bits_invert =
          builder_->createUnaryOp(spv::Op::OpNot, type_uint_, old_depth_stencil);
      builder_->createBranch(false, &block_stencil_op_merge);
      // Increment and wrap.
      ...
      spv::Id new_stencil_in_low_bits_increment_wrap =
          builder_->createBinOp(spv::Op::OpIAdd, type_uint_, old_depth_stencil, const_uint_1);
      builder_->createBranch(false, &block_stencil_op_merge);
      // Decrement and wrap.
      // The upper bits containing the old depth have no effect on the behavior.
      builder_->setBuildPoint(&block_stencil_op_decrement_wrap);
      spv::Id new_stencil_in_low_bits_decrement_wrap =
          builder_->createBinOp(spv::Op::OpISub, type_uint_, old_depth_stencil, const_uint_1);
      builder_->createBranch(false, &block_stencil_op_merge);
      ...
      spv::Id new_stencil_in_low_bits_if_enabled =
          builder_->createOp(spv::Op::OpPhi, type_uint_, id_vector_temp_);
      ...
      new_stencil_and_old_depth_if_stencil_enabled = builder_->createBinOp(
          ...
          builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, new_stencil_in_low_bits_if_enabled,
                                stencil_write_mask));
    }
    ...
    spv::Id depth_stencil_passed =
        builder_->createBinOp(spv::Op::OpLogicalAnd, type_bool_, depth_passed, stencil_passed);
    spv::Id new_sample_mask_after_sample = builder_->createTriOp(
        spv::Op::OpSelect, type_uint_, depth_stencil_passed, new_sample_mask,
        builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, new_sample_mask,
                              builder_->makeUintConstant(~(uint32_t(1) << i))));
    ...
    spv::Id new_stencil_and_unconditional_new_depth =
        builder_->createQuadOp(spv::Op::OpBitFieldInsert, type_uint_, new_stencil_and_old_depth,
                               sample_depth24, const_uint_8, builder_->makeUintConstant(24));
    spv::Id new_depth_stencil = builder_->createTriOp(
        spv::Op::OpSelect, type_uint_,
        builder_->createBinOp(spv::Op::OpLogicalAnd, type_bool_, depth_stencil_passed, depth_write),
    ...
    spv::Id new_depth_stencil_different =
        builder_->createBinOp(spv::Op::OpINotEqual, type_bool_, new_depth_stencil, old_depth_stencil);
    ...
    if (is_early) {
      if (implicit_early_z_write_allowed) {
        new_sample_mask_after_sample = builder_->createTriOp(
            spv::Op::OpSelect, type_uint_,
            builder_->createBinOp(spv::Op::OpLogicalAnd, type_bool_, new_depth_stencil_different,
                                  not_early_write),
            builder_->createBinOp(spv::Op::OpBitwiseOr, type_uint_, new_sample_mask_after_sample,
                                  builder_->makeUintConstant(uint32_t(1) << (4 + i))),
            new_sample_mask_after_sample);
        new_depth_stencil_write_condition = builder_->createBinOp(
            spv::Op::OpLogicalAnd, type_bool_, new_depth_stencil_different, early_write);
      } else {
        // Always need to write late in this shader, as it may do something like
        // explicitly killing pixels.
        new_sample_mask_after_sample = builder_->createTriOp(
            spv::Op::OpSelect, type_uint_, new_depth_stencil_different,
            builder_->createBinOp(spv::Op::OpBitwiseOr, type_uint_, new_sample_mask_after_sample,
                                  builder_->makeUintConstant(uint32_t(1) << (4 + i))),
            new_sample_mask_after_sample);
      }
    ...
    if (new_depth_stencil_write_condition != spv::NoResult) {
      SpirvBuilder::IfBuilder new_depth_stencil_write_if(
          new_depth_stencil_write_condition, spv::SelectionControlMask::DontFlatten, *builder_);
  ...
  if (block_any_sample_covered_merge) {
    builder_->createBranch(false, block_any_sample_covered_merge);
    ...
    new_sample_mask = builder_->createOp(spv::Op::OpPhi, type_uint_, id_vector_temp_);
    if (is_early) {
      for (uint32_t i = 0; i < 4; ++i) {
        id_vector_temp_.clear();
        ...
        id_vector_temp_.push_back(const_uint_0_);
        id_vector_temp_.push_back(block_any_sample_covered_head->getId());
        late_write_depth_stencil[i] = builder_->createOp(spv::Op::OpPhi, type_uint_, id_vector_temp_);
      }
    }
  }
...
std::array<spv::Id, 2> SpirvShaderTranslator::FSI_ClampAndPackColor(spv::Id color_float4,
  ...
  builder_->createSelectionMerge(&block_format_merge, spv::SelectionControlMask::DontFlatten);
  {
  ...
  spv::Id packed_8_8_8_8;
  {
    ...
    spv::Id color_offset = builder_->createNoContractionBinOp(
        spv::Op::OpFAdd, type_float4_, color_scaled, unorm_round_offset_float4);
    spv::Id color_uint4 = builder_->createUnaryOp(spv::Op::OpConvertFToU, type_uint4_, color_offset);
    ...
    for (uint32_t i = 1; i < 4; ++i) {
      packed_8_8_8_8 =
          builder_->createQuadOp(spv::Op::OpBitFieldInsert, type_uint_, packed_8_8_8_8,
                                 builder_->createCompositeExtract(color_uint4, type_uint_, i),
                                 builder_->makeUintConstant(8 * i), component_width);
    }
    builder_->createBranch(false, &block_format_merge);
  }
  ...
  spv::Id packed_8_8_8_8_gamma;
  {
    ...
    spv::Id color_scaled = builder_->createNoContractionBinOp(
        spv::Op::OpVectorTimesScalar, type_float4_, color_gamma, builder_->makeFloatConstant(255.0f));
    spv::Id color_offset = builder_->createNoContractionBinOp(
        spv::Op::OpFAdd, type_float4_, color_scaled, unorm_round_offset_float4);
    spv::Id color_uint4 = builder_->createUnaryOp(spv::Op::OpConvertFToU, type_uint4_, color_offset);
    ...
    for (uint32_t i = 1; i < 4; ++i) {
      packed_8_8_8_8_gamma =
          builder_->createQuadOp(spv::Op::OpBitFieldInsert, type_uint_, packed_8_8_8_8_gamma,
                                 builder_->createCompositeExtract(color_uint4, type_uint_, i),
                                 builder_->makeUintConstant(8 * i), component_width);
    }
    builder_->createBranch(false, &block_format_merge);
  ...
  spv::Id packed_2_10_10_10;
  {
    ...
    spv::Id color_scaled = builder_->createNoContractionBinOp(
        spv::Op::OpFMul, type_float4_, color_clamped,
        builder_->makeCompositeConstant(type_float4_, id_vector_temp_));
    spv::Id color_offset = builder_->createNoContractionBinOp(
        spv::Op::OpFAdd, type_float4_, color_scaled, unorm_round_offset_float4);
    spv::Id color_uint4 = builder_->createUnaryOp(spv::Op::OpConvertFToU, type_uint4_, color_offset);
    ...
    for (uint32_t i = 1; i < 4; ++i) {
      packed_2_10_10_10 = builder_->createQuadOp(
          spv::Op::OpBitFieldInsert, type_uint_, packed_2_10_10_10,
          builder_->createCompositeExtract(color_uint4, type_uint_, i),
  ...
  spv::Id packed_2_10_10_10_float;
  {
    ...
    spv::Id alpha_offset = builder_->createNoContractionBinOp(
        spv::Op::OpFAdd, type_float_, alpha_scaled, unorm_round_offset_float);
    color_components[3] = builder_->createUnaryOp(spv::Op::OpConvertFToU, type_uint_, alpha_offset);
    // Pack.
    packed_2_10_10_10_float = color_components[0];
    spv::Id rgb_width = builder_->makeUintConstant(10);
    for (uint32_t i = 1; i < 3; ++i) {
      packed_2_10_10_10_float = builder_->createQuadOp(
          spv::Op::OpBitFieldInsert, type_uint_, packed_2_10_10_10_float, color_components[i],
          builder_->makeUintConstant(10 * i), rgb_width);
    }
    packed_2_10_10_10_float = builder_->createQuadOp(
        spv::Op::OpBitFieldInsert, type_uint_, packed_2_10_10_10_float, color_components[3],
  ...
  std::array<spv::Id, 2> packed_16;
  {
    ...
    spv::Id color_scaled = builder_->createNoContractionBinOp(
        ...
        builder_->createTriBuiltinCall(
            ...
            builder_->createTriOp(spv::Op::OpSelect, type_float4_,
                                  builder_->createUnaryOp(spv::Op::OpIsNan, type_bool4_, color_float4),
    ...
    spv::Id unorm_round_offset_negative_float4 =
        builder_->makeCompositeConstant(type_float4_, id_vector_temp_);
    spv::Id color_offset = builder_->createNoContractionBinOp(
        spv::Op::OpFAdd, type_float4_, color_scaled,
        builder_->createTriOp(
            spv::Op::OpSelect, type_float4_,
            builder_->createBinOp(spv::Op::OpFOrdLessThan, type_bool4_, color_scaled, const_float4_0_),
    ...
    spv::Id color_uint4 = builder_->createUnaryOp(
        ...
        builder_->createUnaryOp(spv::Op::OpConvertFToS, type_int4_, color_offset));
  ...
  packed[0] = builder_->createOp(spv::Op::OpPhi, type_uint_, id_vector_temp_);
  // High 32 bits.
  id_vector_temp_.clear();
  id_vector_temp_.push_back(const_uint_0_);
  id_vector_temp_.push_back(block_format_8_8_8_8_end.getId());
  ...
  id_vector_temp_.push_back(block_format_32_float_end.getId());
  packed[1] = builder_->createOp(spv::Op::OpPhi, type_uint_, id_vector_temp_);
  return packed;
}
...
std::array<spv::Id, 4> SpirvShaderTranslator::FSI_UnpackColor(std::array<spv::Id, 2> color_packed,
    ...
    for (uint32_t i = 0; i < 2; ++i) {
      ...
      for (uint32_t j = 0; j < 4; ++j) {
        spv::Id component = builder_->createNoContractionBinOp(
            spv::Op::OpFMul, type_float_,
            builder_->createUnaryOp(
                spv::Op::OpConvertUToF, type_float_,
                builder_->createTriOp(spv::Op::OpBitFieldUExtract, type_uint_, color_packed[0],
    ...
    for (uint32_t i = 0; i < 2; ++i) {
      ...
      for (uint32_t j = 0; j <= i; ++j) {
        color_packed_signed[j] =
            builder_->createUnaryOp(spv::Op::OpBitcast, type_int_, color_packed[j]);
      }
    ...
    for (uint32_t i = 0; i < 2; ++i) {
      ...
      for (uint32_t j = 0; j <= i; ++j) {
        unpacked_32_float[i][j] =
            builder_->createUnaryOp(spv::Op::OpBitcast, type_float_, color_packed[j]);
      }
      builder_->createBranch(false, &block_format_merge);
  ...
  for (uint32_t i = 0; i < 4; ++i) {
    ...
    unpacked[i] = builder_->createOp(spv::Op::OpPhi, type_float_, id_vector_temp_);
  }
  return unpacked;
}
...
spv::Id SpirvShaderTranslator::FSI_FlushNaNClampAndInBlending(spv::Id color_or_alpha,
  ...
  SpirvBuilder::IfBuilder if_fixed_point(is_fixed_point, spv::SelectionControlMask::DontFlatten,
                                         *builder_);
  spv::Id color_or_alpha_clamped;
  {
    ...
    color_or_alpha_clamped = builder_->createTriBuiltinCall(
        ...
        builder_->createTriOp(
            spv::Op::OpSelect, color_or_alpha_type,
            builder_->createUnaryOp(spv::Op::OpIsNan, type_bool_vectors_[component_count - 1],
                                    color_or_alpha),
...
spv::Id SpirvShaderTranslator::FSI_ApplyColorBlendFactor(
  ...
  SpirvBuilder::IfBuilder factor_not_zero_if(
      builder_->createBinOp(spv::Op::OpINotEqual, type_bool_, factor,
  ...
  builder_->createSelectionMerge(&block_factor_merge, spv::SelectionControlMask::DontFlatten);
  {
  ...
  builder_->createBranch(false, &block_factor_merge);
  ...
  for (uint32_t i = 0; i < 3; ++i) {
    ...
    // kSrc/Dst/ConstantColor
    {
      ...
      color_factor_results[i] =
          builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float3_, value, color_factor);
      builder_->createBranch(false, &block_factor_merge);
    }
    ...
    // kOneMinusSrc/Dst/ConstantColor
    {
      ...
      one_minus_color_factor_results[i] = builder_->createNoContractionBinOp(
          spv::Op::OpFMul, type_float3_, value,
          builder_->createNoContractionBinOp(spv::Op::OpFSub, type_float3_, const_float3_1_,
                                             color_factor));
      builder_->createBranch(false, &block_factor_merge);
    }
    ...
    // kSrc/Dst/ConstantAlpha
    {
      ...
      alpha_factor_results[i] = builder_->createNoContractionBinOp(
          spv::Op::OpVectorTimesScalar, type_float3_, value, alpha_factor);
      builder_->createBranch(false, &block_factor_merge);
    }
    ...
    // kOneMinusSrc/Dst/ConstantAlpha
    {
      ...
      one_minus_alpha_factor_results[i] = builder_->createNoContractionBinOp(
          spv::Op::OpVectorTimesScalar, type_float3_, value,
          builder_->createNoContractionBinOp(spv::Op::OpFSub, type_float_, const_float_1_,
                                             alpha_factor));
      builder_->createBranch(false, &block_factor_merge);
    }
  }
  ...
  spv::Id result_source_alpha_saturate;
  {
    ...
    result_source_alpha_saturate = builder_->createNoContractionBinOp(
        ...
        builder_->createBinBuiltinCall(type_float_, ext_inst_glsl_std_450_, GLSLstd450NMin,
                                       source_alpha,
                                       builder_->createNoContractionBinOp(
                                           spv::Op::OpFSub, type_float_, const_float_1_, dest_alpha)));
    builder_->createBranch(false, &block_factor_merge);
  }
  ...
  spv::Id result_unclamped = builder_->createOp(spv::Op::OpPhi, type_float3_, id_vector_temp_);
...
spv::Id SpirvShaderTranslator::FSI_ApplyAlphaBlendFactor(spv::Id value, spv::Id is_fixed_point,
  ...
  for (uint32_t i = 0; i < 3; ++i) {
    ...
    // kSrc/Dst/ConstantColor/Alpha
    {
      ...
      alpha_factor_results[i] =
          builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_, value, alpha_factor);
      builder_->createBranch(false, &block_factor_merge);
    }
    ...
    // kOneMinusSrc/Dst/ConstantColor/Alpha
    {
      builder_->setBuildPoint(one_minus_alpha_factor_blocks[i]);
      one_minus_alpha_factor_results[i] = builder_->createNoContractionBinOp(
          spv::Op::OpFMul, type_float_, value,
          builder_->createNoContractionBinOp(spv::Op::OpFSub, type_float_, const_float_1_,
                                             alpha_factor));
      builder_->createBranch(false, &block_factor_merge);
    }
  ...
  id_vector_temp_.push_back(result_source_alpha_saturate);
  id_vector_temp_.push_back(block_factor_source_alpha_saturate.getId());
  spv::Id result_unclamped = builder_->createOp(spv::Op::OpPhi, type_float_, id_vector_temp_);
  spv::Id result = FSI_FlushNaNClampAndInBlending(result_unclamped, is_fixed_point, clamp_min_value,
                                                  clamp_max_value);
...
spv::Id SpirvShaderTranslator::FSI_BlendColorOrAlphaWithUnclampedResult(
  ...
  builder_->createSelectionMerge(&block_min_max_merge, spv::SelectionControlMask::DontFlatten);
  {
  ...
  builder_->createBranch(false, &block_min_max_merge);
  ...
  spv::Id result_factors;
  {
    ...
    builder_->createSelectionMerge(&block_signs_merge, spv::SelectionControlMask::DontFlatten);
    {
    ...
    spv::Id result_add =
        builder_->createNoContractionBinOp(spv::Op::OpFAdd, value_type, term_source, term_dest);
    builder_->createBranch(false, &block_signs_merge);

    // Subtraction case.
    builder_->setBuildPoint(&block_signs_subtract);
    spv::Id result_subtract =
        builder_->createNoContractionBinOp(spv::Op::OpFSub, value_type, term_source, term_dest);
    builder_->createBranch(false, &block_signs_merge);

    // Reverse subtraction case.
    builder_->setBuildPoint(&block_signs_reverse_subtract);
    spv::Id result_reverse_subtract =
        builder_->createNoContractionBinOp(spv::Op::OpFSub, value_type, term_dest, term_source);
    builder_->createBranch(false, &block_signs_merge);
    ...
    result_factors = builder_->createOp(spv::Op::OpPhi, value_type, id_vector_temp_);
    builder_->createBranch(false, &block_min_max_merge);
  }
  ...
  return builder_->createOp(spv::Op::OpPhi, value_type, id_vector_temp_);
}
