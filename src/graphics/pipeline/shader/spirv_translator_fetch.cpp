...
void SpirvShaderTranslator::ProcessVertexFetchInstruction(
  ...
  if (instr.is_mini_fetch) {
    ...
    address = builder_->createUnaryOp(
        ...
        builder_->createBinOp(spv::Op::OpShiftRightLogical, type_uint_, fetch_constant_word_0,
    ...
    if (instr.attributes.stride) {
      ...
      if (instr.attributes.stride > 1) {
        index = builder_->createBinOp(spv::Op::OpIMul, type_int_, index,
                                      builder_->makeIntConstant(int(instr.attributes.stride)));
      }
      address = builder_->createBinOp(spv::Op::OpIAdd, type_int_, address, index);
    }
  ...
  while (rex::bit_scan_forward(words_remaining, &word_index)) {
    ...
    if (word_offset) {
      word_address = builder_->createBinOp(spv::Op::OpIAdd, type_int_, word_address,
  ...
  words = EndianSwap32Uint(
      words, builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, fetch_constant_word_1,
  ...
  switch (instr.attributes.data_format) {
    ...
    case xenos::VertexFormat::k_32_32_32_32:
      ...
      if (instr.attributes.is_signed) {
        result = builder_->createUnaryOp(spv::Op::OpBitcast,
                                         type_int_vectors_[used_format_component_count - 1], words);
        result = builder_->createUnaryOp(spv::Op::OpConvertSToF, result_type, result);
      } else {
        result = builder_->createUnaryOp(spv::Op::OpConvertUToF, result_type, words);
      }
        ...
        if (instr.attributes.is_signed) {
          switch (instr.attributes.signed_rf_mode) {
            ...
            case xenos::SignedRepeatingFractionMode::kNoZero: {
              ...
              result = builder_->createNoContractionBinOp(spv::Op::OpFAdd, result_type, result,
                                                          const_no_zero);
            } break;
            default:
          ...
          result =
              builder_->createNoContractionBinOp(spv::Op::OpVectorTimesScalar, result_type, result,
    ...
    case xenos::VertexFormat::k_32_32_32_FLOAT:
      assert_true(used_format_components == needed_words);
      result = builder_->createUnaryOp(spv::Op::OpBitcast, type_float_vectors_[word_count - 1], words);
      break;
  ...
  if (format_is_packed) {
    ...
    if (instr.attributes.is_signed) {
      ...
      // int.
      words = builder_->createUnaryOp(spv::Op::OpBitcast, type_int_vectors_[word_count - 1], words);
    }
    ...
    for (uint32_t i = 0; i < 4; ++i) {
      ...
      extracted_components[extracted_component_count] = builder_->createTriOp(
          instr.attributes.is_signed ? spv::Op::OpBitFieldSExtract : spv::Op::OpBitFieldUExtract,
    ...
    result = builder_->createUnaryOp(
        instr.attributes.is_signed ? spv::Op::OpConvertSToF : spv::Op::OpConvertUToF, result_type, result);
    // Normalize.
    if (!instr.attributes.is_integer) {
  ...
  if (instr.opcode == ucode::FetchOpcode::kGetTextureGradients) {
    ...
    while (rex::bit_scan_forward(derivative_components_remaining, &derivative_component_index)) {
      ...
      result[derivative_component_index] = builder_->createUnaryOp(
          (derivative_component_index & 0b01) ? spv::Op::OpDPdyCoarse : spv::Op::OpDPdxCoarse, type_float_,
    ...
    if (data_is_3d_needed) {
      ...
      spv::Id fetch_constant_word_5 = builder_->createLoad(
          builder_->createAccessChain(spv::StorageClass::Uniform, uniform_fetch_constants_,
                                      id_vector_temp_),
          spv::NoPrecision);
      spv::Id data_dimension =
          builder_->createTriOp(spv::Op::OpBitFieldUExtract, type_uint_, fetch_constant_word_5,
    ...
    if (size_needed_components) {
      ...
      switch (instr.dimension) {
        case xenos::FetchOpDimension::k1D: {
          if (size_needed_components & 0b1) {
            size[0] = builder_->createTriOp(
                spv::Op::OpBitFieldUExtract, type_uint_, fetch_constant_word_2, const_uint_0_,
        ...
        case xenos::FetchOpDimension::kCube: {
          ...
          if (size_needed_components & 0b10) {
            spv::Id width_height_bit_count =
                builder_->makeUintConstant(xenos::kTexture2DCubeMaxWidthHeightLog2);
            size[1] =
                builder_->createTriOp(spv::Op::OpBitFieldUExtract, type_uint_, fetch_constant_word_2,
        ...
        case xenos::FetchOpDimension::k3DOrStacked: {
          if (size_needed_components & 0b1) {
            ...
            size[0] =
                builder_->createTriOp(spv::Op::OpSelect, type_uint_, data_is_3d, size_3d, size_2d);
          }
          if (size_needed_components & 0b10) {
      ...
      }
      {
        ...
        while (rex::bit_scan_forward(size_remaining_components, &size_component_index)) {
          ...
          size_component_ref = builder_->createBinOp(spv::Op::OpIAdd, type_uint_, size_component_ref,
                                                     builder_->makeUintConstant(1));
          // Convert the size to float for multiplication or division.
          size_component_ref =
              builder_->createUnaryOp(spv::Op::OpConvertUToF, type_float_, size_component_ref);
        }
      }
    }
    ...
    if (revert_resolution_scale_axes) {
      ...
      texture_resolution_scaled = builder_->createBinOp(
          ...
          builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, textures_resolution_scaled,
    ...
    if (instr.opcode == ucode::FetchOpcode::kGetTextureWeights) {
        ...
        for (uint32_t i = 0; i < 2; ++i) {
          if (resolution_scaled_coord_components & (UINT32_C(1) << i)) {
            coordinates[i] = builder_->createTriOp(
                spv::Op::OpSelect, type_float_, texture_resolution_scaled,
                builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_, coordinates[i],
          ...
          if (resolution_scaled_size_components & (UINT32_C(1) << i)) {
            ...
            size[i] = builder_->createTriOp(
                spv::Op::OpSelect, type_float_, texture_resolution_scaled,
                builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_, size[i],
                                                   const_texture_resolution_scale[i]),
                size[i]);
          }
        }
      ...
      while (rex::bit_scan_forward(coordinates_remaining_components, &coordinate_component_index)) {
        ...
        if (!instr.attributes.unnormalized_coordinates) {
          ...
          result_component = builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_,
                                                                result_component, size_component);
        }
        float component_offset = offset_values[coordinate_component_index];
        if (component_offset) {
          result_component =
              builder_->createNoContractionBinOp(spv::Op::OpFAdd, type_float_, result_component,
      ...
      for (uint32_t i = 0; i <= uint32_t(instr.dimension != xenos::FetchOpDimension::k1D); ++i) {
        ...
        if (component_offset != spv::NoResult && texture_resolution_scaled != spv::NoResult &&
          ...
          spv::Id component_offset_resolution_scaled =
              builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_, component_offset,
                                                 const_texture_resolution_scale_reciprocal[i]);
          component_offset =
              builder_->createTriOp(spv::Op::OpSelect, type_float_, texture_resolution_scaled,
                                    component_offset_resolution_scaled, component_offset);
        }
        spv::Id size_component = size[i];
        if (instr.attributes.unnormalized_coordinates) {
          if (component_offset != spv::NoResult) {
            coordinate_ref = builder_->createNoContractionBinOp(spv::Op::OpFAdd, type_float_,
                                                                coordinate_ref, component_offset);
          }
          assert_true(size_component != spv::NoResult);
          coordinate_ref = builder_->createNoContractionBinOp(spv::Op::OpFDiv, type_float_,
                                                              coordinate_ref, size_component);
        } else {
          if (component_offset != spv::NoResult) {
            assert_true(size_component != spv::NoResult);
            spv::Id component_offset_normalized = builder_->createNoContractionBinOp(
                spv::Op::OpFDiv, type_float_, component_offset, size_component);
            coordinate_ref = builder_->createNoContractionBinOp(
                spv::Op::OpFAdd, type_float_, coordinate_ref, component_offset_normalized);
          }
        }
      }
      if (instr.dimension == xenos::FetchOpDimension::k3DOrStacked) {
        ...
        if (instr.attributes.unnormalized_coordinates) {
          ...
          if (z_offset != spv::NoResult) {
            z_coordinate_ref = builder_->createNoContractionBinOp(spv::Op::OpFAdd, type_float_,
                                                                  z_coordinate_ref, z_offset);
          }
          assert_true(data_is_3d != spv::NoResult);
          SpirvBuilder::IfBuilder if_data_is_3d(data_is_3d, spv::SelectionControlMask::DontFlatten,
                                                *builder_);
          spv::Id z_3d;
          {
            assert_true(z_size != spv::NoResult);
            z_3d = builder_->createNoContractionBinOp(spv::Op::OpFDiv, type_float_, z_coordinate_ref,
                                                      z_size);
          }
          ...
          if (block_dimension_3d) {
            ...
            if (z_offset != spv::NoResult) {
              ...
              spv::Id z_offset_normalized =
                  builder_->createNoContractionBinOp(spv::Op::OpFDiv, type_float_, z_offset, z_size);
              z_3d = builder_->createNoContractionBinOp(spv::Op::OpFAdd, type_float_, z_3d,
                                                        z_offset_normalized);
            }
            builder_->createBranch(false, &block_dimension_merge);
          }
          // Stacked case.
          ...
          assert_true(z_size != spv::NoResult);
          z_stacked =
              builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_, z_stacked, z_size);
          if (z_offset != spv::NoResult) {
            z_stacked =
                builder_->createNoContractionBinOp(spv::Op::OpFAdd, type_float_, z_stacked, z_offset);
          }
          builder_->createBranch(false, &block_dimension_merge);
        ...
        if (offset_values[2]) {
          face = builder_->createNoContractionBinOp(spv::Op::OpFAdd, type_float_, face,
                                                    builder_->makeFloatConstant(offset_values[2]));
        }
        face =
            builder_->createUnaryOp(spv::Op::OpConvertFToU, type_uint_,
        ...
        spv::Id face_axis =
            builder_->createBinOp(spv::Op::OpShiftRightLogical, type_uint_, face, const_uint_1);
        spv::Id face_is_negative = builder_->createBinOp(
            spv::Op::OpINotEqual, type_bool_,
            builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, face, const_uint_1),
            const_uint_0_);
        spv::Id face_sign = builder_->createTriOp(spv::Op::OpSelect, type_float_, face_is_negative,
                                                  builder_->makeFloatConstant(-1.0f),
                                                  builder_->makeFloatConstant(1.0f));
        // Remap the axes in a way opposite to the ALU cube instruction.
        spv::Id sc_negated =
            builder_->createNoContractionUnaryOp(spv::Op::OpFNegate, type_float_, coordinates[0]);
        spv::Id tc_negated =
            builder_->createNoContractionUnaryOp(spv::Op::OpFNegate, type_float_, coordinates[1]);
        ...
        builder_->createSelectionMerge(&block_ma_merge, spv::SelectionControlMask::MaskNone);
        {
        ...
        spv::Id ma_x_z = builder_->createTriOp(spv::Op::OpSelect, type_float_, face_is_negative,
                                               coordinates[0], sc_negated);
        builder_->createBranch(false, &block_ma_merge);
        // Y is the major axis case.
        builder_->setBuildPoint(&block_ma_y);
        spv::Id ma_y_x = coordinates[0];
        spv::Id ma_y_z = builder_->createTriOp(spv::Op::OpSelect, type_float_, face_is_negative,
                                               tc_negated, coordinates[1]);
        builder_->createBranch(false, &block_ma_merge);
        // Z is the major axis case.
        builder_->setBuildPoint(&block_ma_z);
        spv::Id ma_z_x = builder_->createTriOp(spv::Op::OpSelect, type_float_, face_is_negative,
        ...
        builder_->setBuildPoint(&block_ma_merge);
        {
          std::unique_ptr<spv::Instruction> x_phi_op =
              std::make_unique<spv::Instruction>(builder_->getUniqueId(), type_float_, spv::Op::OpPhi);
      ...
      if (instr.opcode == ucode::FetchOpcode::kGetTextureComputedLod) {
        ...
        spv::Id swizzled_signs_all_signed = builder_->createBinOp(
            ...
            builder_->createTriOp(spv::Op::OpBitFieldUExtract, type_uint_, swizzled_signs_word,
            ...
            builder_->makeUintConstant(uint32_t(xenos::TextureSign::kSigned));
        {
          ...
          while (rex::bit_scan_forward(result_remaining_components, &result_component_index)) {
            ...
            spv::Id is_component_signed = builder_->createBinOp(
                spv::Op::OpIEqual, type_bool_, result_component_sign, const_uint_sign_signed);
            result_is_signed[result_component_index] = is_component_signed;
            if (is_all_signed != spv::NoResult) {
              is_all_signed = builder_->createBinOp(spv::Op::OpLogicalAnd, type_bool_, is_all_signed,
                                                    is_component_signed);
            } else {
              is_all_signed = is_component_signed;
            }
            if (is_any_signed != spv::NoResult) {
              is_any_signed = builder_->createBinOp(spv::Op::OpLogicalOr, type_bool_, is_any_signed,
                                                    is_component_signed);
            } else {
        ...
        spv::Id is_any_unsigned =
            builder_->createUnaryOp(spv::Op::OpLogicalNot, type_bool_, is_all_signed);
        ...
        spv::Id fetch_constant_word_4_signed =
            builder_->createUnaryOp(spv::Op::OpBitcast, type_int_, fetch_constant_word_4);
            ...
            case xenos::FetchOpDimension::k1D: {
              ...
              if (instr.attributes.use_register_gradients) {
                ...
                if (instr.attributes.unnormalized_coordinates) {
                  ...
                  gradient_h_1d = builder_->createNoContractionBinOp(spv::Op::OpFDiv, type_float_,
                                                                     gradient_h_1d, size[0]);
                  gradient_v_1d = builder_->createNoContractionBinOp(spv::Op::OpFDiv, type_float_,
                                                                     gradient_v_1d, size[0]);
                }
              } else {
                builder_->addCapability(spv::Capability::DerivativeControl);
                gradient_h_1d =
                    builder_->createUnaryOp(spv::Op::OpDPdxCoarse, type_float_, coordinates[0]);
                gradient_v_1d =
                    builder_->createUnaryOp(spv::Op::OpDPdyCoarse, type_float_, coordinates[0]);
              }
              gradient_h_1d = builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_,
                                                                 gradient_h_1d, lod_gradient_scale);
              gradient_v_1d = builder_->createNoContractionBinOp(spv::Op::OpFMul, type_float_,
            ...
            case xenos::FetchOpDimension::k2D: {
              if (instr.attributes.use_register_gradients) {
                for (uint32_t i = 0; i < 2; ++i) {
                  ...
                  if (instr.attributes.unnormalized_coordinates) {
                    ...
                    register_gradient_x = builder_->createNoContractionBinOp(
                        spv::Op::OpFDiv, type_float_, register_gradient_x, size[0]);
                    assert_true(size[1] != spv::NoResult);
                    register_gradient_y = builder_->createNoContractionBinOp(
                        spv::Op::OpFDiv, type_float_, register_gradient_y, size[1]);
                  }
                  id_vector_temp_.clear();
                ...
                spv::Id gradient_coordinate_vector =
                    builder_->createCompositeConstruct(type_float2_, id_vector_temp_);
                builder_->addCapability(spv::Capability::DerivativeControl);
                gradients_h = builder_->createUnaryOp(spv::Op::OpDPdxCoarse, type_float2_,
                                                      gradient_coordinate_vector);
                gradients_v = builder_->createUnaryOp(spv::Op::OpDPdyCoarse, type_float2_,
                                                      gradient_coordinate_vector);
              }
              gradients_h = builder_->createNoContractionBinOp(
                  spv::Op::OpVectorTimesScalar, type_float2_, gradients_h, lod_gradient_scale);
              gradients_v = builder_->createNoContractionBinOp(
                  spv::Op::OpVectorTimesScalar, type_float2_, gradients_v, lod_gradient_scale);
            } break;
            case xenos::FetchOpDimension::k3DOrStacked: {
              if (instr.attributes.use_register_gradients) {
                ...
                spv::Id gradient_coordinate_vector =
                    builder_->createCompositeConstruct(type_float3_, id_vector_temp_);
                builder_->addCapability(spv::Capability::DerivativeControl);
                gradients_h = builder_->createUnaryOp(spv::Op::OpDPdxCoarse, type_float3_,
                                                      gradient_coordinate_vector);
                gradients_v = builder_->createUnaryOp(spv::Op::OpDPdyCoarse, type_float3_,
                                                      gradient_coordinate_vector);
              }
              gradients_h = builder_->createNoContractionBinOp(
                  spv::Op::OpVectorTimesScalar, type_float3_, gradients_h, lod_gradient_scale);
              gradients_v = builder_->createNoContractionBinOp(
                  spv::Op::OpVectorTimesScalar, type_float3_, gradients_v, lod_gradient_scale);
            } break;
            case xenos::FetchOpDimension::kCube: {
              if (instr.attributes.use_register_gradients) {
        ...
        spv::ImageOperandsMask image_operands_mask =
            use_computed_lod ? spv::ImageOperandsMask::Grad : spv::ImageOperandsMask::Lod;
          ...
          spv::Id sample_result_unsigned_stacked, sample_result_signed_stacked;
          {
            ...
            if (use_computed_lod &&
              ...
              spv::Id is_minifying_z =
                  builder_->createBinOp(spv::Op::OpFOrdLessThan, type_bool_, layer_max_gradient,
                                        builder_->makeFloatConstant(1.0f));
              ...
              vol_filter_is_linear = builder_->createTriOp(
                  spv::Op::OpSelect, type_bool_, is_minifying_z, vol_min_filter_is_linear_loaded,
              ...
              if (vol_mag_filter_is_fetch_const) {
                vol_filter_is_linear = builder_->createBinOp(
                    spv::Op::OpINotEqual, type_bool_,
                    builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, fetch_constant_word_4,
                                          builder_->makeUintConstant(UINT32_C(1) << 0)),
                    const_uint_0_);
              }
            }
            ...
            if (vol_filter_is_linear != spv::NoResult) {
              layer_coordinate = builder_->createTriOp(
                  ...
                  builder_->createNoContractionBinOp(spv::Op::OpFSub, type_float_, layer_coordinate,
                                                     builder_->makeFloatConstant(0.5f)),
                  layer_coordinate);
            } else if (vol_mag_filter_is_linear) {
              layer_coordinate = builder_->createNoContractionBinOp(
                  spv::Op::OpFSub, type_float_, layer_coordinate, builder_->makeFloatConstant(0.5f));
            }
            ...
            if (vol_filter_is_linear != spv::NoResult || vol_mag_filter_is_linear) {
              ...
              spv::Id layer_1_coordinate = builder_->createBinOp(
                  spv::Op::OpFAdd, type_float_, layer_0_coordinate, builder_->makeFloatConstant(1.0f));
              id_vector_temp_.clear();
              id_vector_temp_.push_back(coordinates[0]);
              id_vector_temp_.push_back(coordinates[1]);
              ...
              if (vol_filter_is_linear != spv::NoResult) {
                ...
                builder_->createBranch(false, &block_z_merge);
        ...
        if (!features_.image_view_format_swizzle) {
          ...
          while (rex::bit_scan_forward(result_remaining_components, &result_component_index)) {
            ...
            spv::Id swizzle_bit_0 = builder_->createBinOp(
                spv::Op::OpINotEqual, type_bool_,
                builder_->createBinOp(spv::Op::OpBitwiseAnd, type_uint_, swizzle_word,
            ...
            SpirvBuilder::IfBuilder if_swizzle_constant(
                swizzle_bit_2, spv::SelectionControlMask::DontFlatten, *builder_);
            spv::Id swizzle_result_constant;
            {
              // Constant values.
              // Bit 0 - 0 or 1.
              swizzle_result_constant = builder_->createTriOp(
                  spv::Op::OpSelect, type_float_, swizzle_bit_0, const_float_1, const_float_0_);
            }
            ...
            spv::Id swizzle_result_component;
            {
              ...
              swizzle_result_component = builder_->createTriOp(
                  spv::Op::OpSelect, type_float_, swizzle_bit_1, swizzle_z_or_w, swizzle_x_or_y);
            }
        ...
        spv::Id const_float_minus_1 = builder_->makeFloatConstant(-1.0f);
        {
          ...
          while (rex::bit_scan_forward(result_remaining_components, &result_component_index)) {
            ...
            builder_->createSelectionMerge(&block_sign_merge, spv::SelectionControlMask::DontFlatten);
            {
            ...
            if (block_sign_signed) {
              ...
              builder_->createBranch(false, &block_sign_merge);
            }
            // Unsigned biased.
            ...
            spv::Id sample_result_component_unsigned_biased = builder_->createNoContractionBinOp(
                spv::Op::OpFMul, type_float_, sample_result_component_unsigned, const_float_2);
            sample_result_component_unsigned_biased = builder_->createNoContractionBinOp(
                spv::Op::OpFAdd, type_float_, sample_result_component_unsigned_biased,
                const_float_minus_1);
            builder_->createBranch(false, &block_sign_merge);
            // Gamma.
        ...
        spv::Id result_exponent_bias = builder_->createBinBuiltinCall(
            ...
            builder_->createTriOp(spv::Op::OpBitFieldSExtract, type_int_, fetch_constant_word_4_signed,
                                  builder_->makeUintConstant(13), builder_->makeUintConstant(6)));
        {
          ...
          while (rex::bit_scan_forward(result_remaining_components, &result_component_index)) {
            ...
            result[result_component_index] = builder_->createNoContractionBinOp(
                spv::Op::OpFMul, type_float_, result[result_component_index], result_exponent_bias);
          }
        }
      }
    }
...
size_t SpirvShaderTranslator::FindOrAddTextureBinding(uint32_t fetch_constant,
  ...
  builder_->addDecoration(new_texture_binding.variable, spv::Decoration::Binding,
...
size_t SpirvShaderTranslator::FindOrAddSamplerBinding(uint32_t fetch_constant,
  ...
  new_sampler_binding.variable =
      builder_->createVariable(spv::NoPrecision, spv::StorageClass::UniformConstant,
  ...
  for (uint32_t i = 0; i < 2; ++i) {
    ...
    spv::Id sign_result;
    {
      ...
      texture_parameters.sampler = builder_->createBinOp(
          spv::Op::OpSampledImage, builder_->makeSampledImageType(builder_->getTypeId(image)), image,
          sampler);
      sign_result =
      ...
      if (lerp_factor != spv::NoResult) {
        ...
        if (lerp_first != spv::NoResult) {
          spv::Id lerp_difference = builder_->createNoContractionBinOp(
              ...
              builder_->createNoContractionBinOp(spv::Op::OpFSub, type_float4_, sign_result,
                                                 lerp_first),
              lerp_factor);
          sign_result = builder_->createNoContractionBinOp(spv::Op::OpFAdd, type_float4_, sign_result,
                                                           lerp_difference);
        }
      }
    }
...
spv::Id SpirvShaderTranslator::QueryTextureLod(spv::Builder::TextureParameters& texture_parameters,
  ...
  // OpSampledImage must be in the same block as where its result is used.
  SpirvBuilder::IfBuilder if_signed(is_all_signed, spv::SelectionControlMask::DontFlatten, *builder_);
  spv::Id lod_signed;
  {
    texture_parameters.sampler = builder_->createBinOp(
        spv::Op::OpSampledImage, builder_->makeSampledImageType(builder_->getTypeId(image_signed)),
        image_signed, sampler);
    lod_signed = builder_->createCompositeExtract(
        builder_->createTextureQueryCall(spv::Op::OpImageQueryLod, texture_parameters, false),
        type_float_, 1);
  }
  ...
  {
    texture_parameters.sampler = builder_->createBinOp(
        spv::Op::OpSampledImage, builder_->makeSampledImageType(builder_->getTypeId(image_unsigned)),
