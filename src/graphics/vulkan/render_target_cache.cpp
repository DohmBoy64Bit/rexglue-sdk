...
VkShaderModule VulkanRenderTargetCache::GetTransferShader(TransferShaderKey key) {
  ...
  switch (mode.output) {
    case TransferOutput::kColor:
      ...
      output_fragment_data =
          builder.createVariable(spv::NoPrecision, spv::StorageClass::Output, type_fragment_data,
                                 "xe_transfer_fragment_data");
      builder.addDecoration(output_fragment_data, spv::Decoration::Location, key.dest_color_rt_index);
  ...
  if (pipeline_layout_info.used_descriptor_sets & kTransferUsedDescriptorSetColorTextureBit) {
    source_color_texture = builder.createVariable(
        ...
        builder.makeImageType(source_color_component_type, spv::Dim::Dim2D, false, false,
                              source_is_multisampled, 1, spv::ImageFormat::Unknown),
        "xe_transfer_color");
    builder.addDecoration(source_color_texture, spv::Decoration::DescriptorSet,
  ...
  if (pipeline_layout_info.used_descriptor_sets &
    ...
    if (mode.output != TransferOutput::kStencilBit) {
      source_depth_texture = builder.createVariable(
          spv::NoPrecision, spv::StorageClass::UniformConstant,
          builder.makeImageType(type_float, spv::Dim::Dim2D, false, false, source_is_multisampled, 1,
                                spv::ImageFormat::Unknown),
          "xe_transfer_depth");
      builder.addDecoration(source_depth_texture, spv::Decoration::DescriptorSet,
    ...
    if (mode.output != TransferOutput::kDepth || shader_uses_stencil_reference_output) {
      source_stencil_texture = builder.createVariable(
          spv::NoPrecision, spv::StorageClass::UniformConstant,
          builder.makeImageType(type_uint, spv::Dim::Dim2D, false, false, source_is_multisampled, 1,
                                spv::ImageFormat::Unknown),
          "xe_transfer_stencil");
      builder.addDecoration(source_stencil_texture, spv::Decoration::DescriptorSet,
                            source_depth_stencil_descriptor_set);
  ...
  if (pipeline_layout_info.used_descriptor_sets & kTransferUsedDescriptorSetHostDepthBufferBit) {
    ...
    builder.addDecoration(id_vector_temp.back(), spv::Decoration::ArrayStride, sizeof(uint32_t));
    ...
    builder.addMemberDecoration(type_host_depth_source_buffer, 0, spv::Decoration::NonWritable);
    builder.addMemberDecoration(type_host_depth_source_buffer, 0, spv::Decoration::Offset, 0);
    // Block since SPIR-V 1.3, but since SPIR-V 1.0 is generated, it's
    // BufferBlock.
    builder.addDecoration(type_host_depth_source_buffer, spv::Decoration::BufferBlock);
    // StorageBuffer since SPIR-V 1.3, but since SPIR-V 1.0 is generated, it's
    // Uniform.
    host_depth_source_buffer =
        builder.createVariable(spv::NoPrecision, spv::StorageClass::Uniform,
                               type_host_depth_source_buffer, "xe_transfer_host_depth_buffer");
    builder.addDecoration(host_depth_source_buffer, spv::Decoration::DescriptorSet,
                          rex::bit_count(pipeline_layout_info.used_descriptor_sets &
                                         (kTransferUsedDescriptorSetHostDepthBufferBit - 1)));
    builder.addDecoration(host_depth_source_buffer, spv::Decoration::Binding, 0);
  }
  ...
  if (pipeline_layout_info.used_descriptor_sets &
    ...
    builder.addDecoration(host_depth_source_texture, spv::Decoration::Binding, 0);
  }
  // Push constants.
  id_vector_temp.clear();
  ...
  if (!id_vector_temp.empty()) {
    ...
    if (pipeline_layout_info.used_push_constant_dwords &
      ...
      builder.addMemberDecoration(
          type_push_constants, push_constants_member_host_depth_address, spv::Decoration::Offset,
          sizeof(uint32_t) *
    ...
    if (pipeline_layout_info.used_push_constant_dwords & kTransferUsedPushConstantDwordAddressBit) {
      ...
      builder.addMemberDecoration(
          type_push_constants, push_constants_member_address, spv::Decoration::Offset,
    ...
    if (pipeline_layout_info.used_push_constant_dwords &
      ...
      builder.addMemberName(type_push_constants, push_constants_member_stencil_mask,
                            "stencil_mask");
      builder.addMemberDecoration(
          type_push_constants, push_constants_member_stencil_mask, spv::Decoration::Offset,
          sizeof(uint32_t) * rex::bit_count(pipeline_layout_info.used_push_constant_dwords &
                                            (kTransferUsedPushConstantDwordStencilMaskBit - 1)));
    }
    builder.addDecoration(type_push_constants, spv::Decoration::Block);
    push_constants = builder.createVariable(spv::NoPrecision, spv::StorageClass::PushConstant,
  ...
  spv::Id dest_tile_index_x =
      builder.createBinOp(spv::Op::OpUDiv, type_uint, dest_pixel_x, const_dest_tile_width_pixels);
  spv::Id dest_tile_pixel_x =
      builder.createBinOp(spv::Op::OpUMod, type_uint, dest_pixel_x, const_dest_tile_width_pixels);
  ...
  spv::Id dest_tile_index_y =
      builder.createBinOp(spv::Op::OpUDiv, type_uint, dest_pixel_y, const_dest_tile_height_pixels);
  spv::Id dest_tile_pixel_y =
      builder.createBinOp(spv::Op::OpUMod, type_uint, dest_pixel_y, const_dest_tile_height_pixels);
  ...
  spv::Id address_constant = builder.createLoad(
      builder.createAccessChain(spv::StorageClass::PushConstant, push_constants, id_vector_temp),
      spv::NoPrecision);
  ...
  spv::Id dest_tile_index = builder.createBinOp(
      ...
      builder.createBinOp(
          ...
          builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, address_constant,
  ...
  if (key.dest_msaa_samples != xenos::MsaaSamples::k1X) {
    if (device_properties.sampleRateShading) {
      ...
      dest_sample_id = builder.createUnaryOp(spv::Op::OpBitcast, type_uint,
  ...
  if (!source_is_64bpp && dest_is_64bpp) {
    ...
    if (key.source_msaa_samples >= xenos::MsaaSamples::k4X) {
      ...
      if (key.dest_msaa_samples >= xenos::MsaaSamples::k4X) {
        ...
        source_sample_id = builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, dest_sample_id,
                                               builder.makeUintConstant(1 << 1));
        source_tile_pixel_x = builder.createQuadOp(spv::Op::OpBitFieldInsert, type_uint, dest_sample_id,
        ...
        if (msaa_2x_attachments_supported_) {
          source_sample_id =
              builder.createBinOp(spv::Op::OpShiftLeftLogical, type_uint,
                                  builder.createBinOp(spv::Op::OpBitwiseXor, type_uint, dest_sample_id,
        ...
        source_sample_id = builder.createQuadOp(
            spv::Op::OpBitFieldInsert, type_uint, builder.makeUintConstant(0), dest_tile_pixel_y,
            builder.makeUintConstant(1), builder.makeUintConstant(1));
        source_tile_pixel_y = builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint,
      ...
      if (key.dest_msaa_samples >= xenos::MsaaSamples::k4X) {
        ...
        source_tile_pixel_x = builder.createQuadOp(
            ...
            builder.createBinOp(spv::Op::OpShiftLeftLogical, type_uint, dest_tile_pixel_x,
        ...
        source_tile_pixel_x = builder.createBinOp(spv::Op::OpShiftLeftLogical, type_uint,
                                                  dest_tile_pixel_x, builder.makeUintConstant(1));
        // Y is handled by common code.
      }
    }
    ...
    if (key.dest_msaa_samples >= xenos::MsaaSamples::k4X) {
      ...
      if (key.source_msaa_samples >= xenos::MsaaSamples::k4X) {
        ...
        source_sample_id = builder.createQuadOp(spv::Op::OpBitFieldInsert, type_uint, dest_sample_id,
        ...
        source_tile_pixel_x = builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint,
                                                  dest_tile_pixel_x, builder.makeUintConstant(1));
      } else {
      ...
      source_color_half = builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, dest_sample_id,
                                              builder.makeUintConstant(1));
    } else {
      // 64bpp -> 32bpp, -> 1x/2x.
      ...
      if (key.source_msaa_samples >= xenos::MsaaSamples::k4X) {
        ...
        // (first bit).
        source_sample_id =
            builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, dest_tile_pixel_x,
                                builder.makeUintConstant(1), builder.makeUintConstant(1));
        if (key.dest_msaa_samples == xenos::MsaaSamples::k2X) {
          ...
          if (msaa_2x_attachments_supported_) {
            ...
            source_sample_id = builder.createQuadOp(
                spv::Op::OpBitFieldInsert, type_uint, dest_sample_id, source_sample_id,
          ...
          source_sample_id = builder.createQuadOp(
              spv::Op::OpBitFieldInsert, type_uint, source_sample_id, source_tile_pixel_y,
              builder.makeUintConstant(1), builder.makeUintConstant(1));
          source_tile_pixel_y = builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint,
                                                    dest_tile_pixel_y, builder.makeUintConstant(1));
        }
      ...
      source_color_half = builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, dest_tile_pixel_x,
                                              builder.makeUintConstant(1));
    }
      ...
      if (key.source_msaa_samples >= xenos::MsaaSamples::k4X) {
        ...
        if (key.dest_msaa_samples == xenos::MsaaSamples::k2X) {
          ...
          if (msaa_2x_attachments_supported_) {
            ...
            source_sample_id = builder.createQuadOp(
                spv::Op::OpBitFieldInsert, type_uint, dest_sample_id, dest_tile_pixel_x,
                builder.makeUintConstant(0), builder.makeUintConstant(1));
          }
          source_tile_pixel_x = builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint,
                                                    dest_tile_pixel_x, builder.makeUintConstant(1));
          ...
          source_sample_id = builder.createQuadOp(
              spv::Op::OpBitFieldInsert, type_uint,
              builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, dest_tile_pixel_x,
                                  builder.makeUintConstant(1)),
  ...
  if (key.source_msaa_samples < xenos::MsaaSamples::k4X &&
    ...
    if (key.dest_msaa_samples >= xenos::MsaaSamples::k4X) {
      ...
      if (key.source_msaa_samples == xenos::MsaaSamples::k2X) {
        ...
        source_sample_id = builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, dest_sample_id,
                                               builder.makeUintConstant(1));
        if (msaa_2x_attachments_supported_) {
          source_sample_id = builder.createBinOp(spv::Op::OpBitwiseXor, type_uint, source_sample_id,
                                                 builder.makeUintConstant(1));
        } else {
          source_sample_id = builder.createQuadOp(
              spv::Op::OpBitFieldInsert, type_uint, source_sample_id, source_sample_id,
              builder.makeUintConstant(1), builder.makeUintConstant(1));
        }
      } else {
        // 1x -> 4x.
        ...
        source_tile_pixel_y = builder.createQuadOp(
            spv::Op::OpBitFieldInsert, type_uint,
            builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, dest_sample_id,
                                builder.makeUintConstant(1)),
      ...
      if (key.source_msaa_samples == xenos::MsaaSamples::k2X) {
        ...
        source_sample_id = builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, dest_tile_pixel_y,
                                               builder.makeUintConstant(1));
        if (msaa_2x_attachments_supported_) {
          source_sample_id = builder.createBinOp(spv::Op::OpBitwiseXor, type_uint, source_sample_id,
                                                 builder.makeUintConstant(1));
  ...
  if (source_is_color != dest_is_color) {
    ...
    source_tile_pixel_x = builder.createUnaryOp(
        ...
        builder.createBinOp(
            ...
            builder.createUnaryOp(spv::Op::OpBitcast, type_int, source_tile_pixel_x),
            builder.createTriOp(
                spv::Op::OpSelect, type_int,
                builder.createBinOp(spv::Op::OpULessThan, builder.makeBoolType(), source_tile_pixel_x,
  ...
  spv::Id source_tile_index = builder.createBinOp(
      ...
      builder.createUnaryOp(
          ...
          builder.createBinOp(
              spv::Op::OpIAdd, type_int,
              builder.createUnaryOp(spv::Op::OpBitcast, type_int, dest_tile_index),
              builder.createTriOp(spv::Op::OpBitFieldSExtract, type_int,
                                  builder.createUnaryOp(spv::Op::OpBitcast, type_int, address_constant),
  ...
  spv::Id source_tile_index_y =
      builder.createBinOp(spv::Op::OpUDiv, type_uint, source_tile_index, source_pitch_tiles);
  spv::Id source_tile_index_x =
      builder.createBinOp(spv::Op::OpUMod, type_uint, source_tile_index, source_pitch_tiles);
  ...
  if (key.source_msaa_samples != xenos::MsaaSamples::k1X) {
    source_sample_ids_int[0] = builder.createUnaryOp(spv::Op::OpBitcast, type_int, source_sample_id);
  } else {
  ...
  if (source_load_is_two_32bpp_samples) {
    if (key.source_msaa_samples >= xenos::MsaaSamples::k4X) {
      ...
      source_sample_ids_int[1] = builder.createBinOp(
          spv::Op::OpBitwiseOr, type_int, source_sample_ids_int[0], builder.makeIntConstant(1));
    } else {
      id_vector_temp.clear();
      id_vector_temp.push_back(builder.createBinOp(spv::Op::OpBitwiseOr, type_int, source_pixel_x_int,
  ...
  if (source_is_64bpp && !dest_is_64bpp) {
    ...
    spv::Id source_color_is_second_half = builder.createBinOp(
        spv::Op::OpINotEqual, type_bool, source_color_half, builder.makeUintConstant(0));
    if (mode.output == TransferOutput::kStencilBit) {
      source_color[0][0] = builder.createTriOp(
          spv::Op::OpSelect, source_color_component_type, source_color_is_second_half,
  ...
  if (output_fragment_stencil_ref != spv::NoResult && source_stencil[0] != spv::NoResult) {
    ...
    builder.createStore(builder.createUnaryOp(spv::Op::OpBitcast, type_int, source_stencil[0]),
  ...
  auto PWLGammaToLinear = [&](spv::Id gamma, bool gamma_pre_saturated) -> spv::Id {
    ...
    spv::Id is_piece_at_least_3 = builder.createBinOp(spv::Op::OpFOrdGreaterThanEqual, type_bool, gamma,
                                                      builder.makeFloatConstant(192.0f / 255.0f));
    spv::Id scale_3_or_2 = builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_3,
                                               builder.makeFloatConstant(8.0f / 1024.0f),
                                               builder.makeFloatConstant(4.0f / 1024.0f));
    spv::Id offset_3_or_2 = builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_3,
                                                builder.makeFloatConstant(-1024.0f),
                                                builder.makeFloatConstant(-256.0f));
    spv::Id is_piece_at_least_1 = builder.createBinOp(spv::Op::OpFOrdGreaterThanEqual, type_bool, gamma,
                                                      builder.makeFloatConstant(64.0f / 255.0f));
    spv::Id scale_1_or_0 = builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_1,
                                               builder.makeFloatConstant(2.0f / 1024.0f),
                                               builder.makeFloatConstant(1.0f / 1024.0f));
    spv::Id offset_1_or_0 = builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_1,
                                                builder.makeFloatConstant(-64.0f), const_float_0);
    spv::Id is_piece_at_least_2 = builder.createBinOp(spv::Op::OpFOrdGreaterThanEqual, type_bool, gamma,
                                                      builder.makeFloatConstant(96.0f / 255.0f));
    spv::Id scale = builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_2,
                                        scale_3_or_2, scale_1_or_0);
    spv::Id offset = builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_2,
    ...
    spv::Id linear = builder.createBinOp(
        ...
        builder.createBinOp(spv::Op::OpFMul, type_float,
                            builder.createBinOp(spv::Op::OpFMul, type_float, gamma,
                                                builder.makeFloatConstant(255.0f * 1024.0f)),
                            scale),
        offset);
    linear = builder.createBinOp(spv::Op::OpFAdd, type_float, linear,
                                 builder.createUnaryBuiltinCall(
                                     type_float, ext_inst_glsl_std_450, GLSLstd450Trunc,
                                     builder.createBinOp(spv::Op::OpFMul, type_float, linear, scale)));
    return builder.createBinOp(spv::Op::OpFMul, type_float, linear,
  ...
  auto LinearToPWLGamma = [&](spv::Id linear, bool linear_pre_saturated) -> spv::Id {
    ...
    spv::Id is_piece_at_least_3 =
        builder.createBinOp(spv::Op::OpFOrdGreaterThanEqual, type_bool, linear,
    ...
    spv::Id is_piece_at_least_1 = builder.createBinOp(
        spv::Op::OpFOrdGreaterThanEqual, type_bool, linear, builder.makeFloatConstant(64.0f / 1023.0f));
    spv::Id scale_1_or_0 = builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_1,
                                               builder.makeFloatConstant(1023.0f / 2.0f),
                                               builder.makeFloatConstant(1023.0f));
    spv::Id offset_1_or_0 =
        builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_1,
  ...
  auto PackSource16ComponentToUint = [&](spv::Id component) -> spv::Id {
    ...
    if (source_color_16_is_float) {
      ...
      return builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, packed_half, const_uint_0,
                                 const_uint_16);
    }
    ...
    spv::Id component_rounded = builder.createUnaryBuiltinCall(
        ...
        builder.createBinOp(spv::Op::OpFMul, type_float, component_clamped, const_float_32767));
    spv::Id component_snorm =
        builder.createUnaryOp(spv::Op::OpConvertFToS, type_int, component_rounded);
    return builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint,
                               builder.createUnaryOp(spv::Op::OpBitcast, type_uint, component_snorm),
  ...
  auto PackSource16PairToUint32 = [&](spv::Id component_0, spv::Id component_1) -> spv::Id {
    return builder.createQuadOp(
        spv::Op::OpBitFieldInsert, type_uint, PackSource16ComponentToUint(component_0),
  ...
  auto UnpackDest16ComponentFromUint32 = [&](spv::Id packed_word,
    ...
    if (dest_color_is_uint) {
      return builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, packed_word, component_offset,
                                 const_uint_16);
    }
    ...
    spv::Id component_snorm = builder.createTriOp(spv::Op::OpBitFieldSExtract, type_int, packed_word,
                                                  component_offset, const_uint_16);
    spv::Id component_float =
        builder.createBinOp(spv::Op::OpFMul, type_float,
                            builder.createUnaryOp(spv::Op::OpConvertSToF, type_float, component_snorm),
  ...
  if (dest_is_64bpp) {
    ...
    if (source_is_color) {
      switch (source_color_format) {
        ...
        case xenos::ColorRenderTargetFormat::k_8_8_8_8: {
          ...
          for (uint32_t i = 0; i < 2; ++i) {
            packed[i] = builder.createUnaryOp(
                ...
                builder.createBinOp(
                    spv::Op::OpFAdd, type_float,
                    builder.createBinOp(spv::Op::OpFMul, type_float, source_color[i][0], unorm_scale),
                    unorm_round_offset));
            for (uint32_t j = 1; j < 4; ++j) {
              packed[i] = builder.createQuadOp(
                  ...
                  builder.createUnaryOp(
                      spv::Op::OpConvertFToU, type_uint,
                      builder.createBinOp(spv::Op::OpFAdd, type_float,
                                          builder.createBinOp(spv::Op::OpFMul, type_float,
        ...
        case xenos::ColorRenderTargetFormat::k_2_10_10_10_AS_10_10_10_10: {
          ...
          for (uint32_t i = 0; i < 2; ++i) {
            packed[i] = builder.createUnaryOp(
                spv::Op::OpConvertFToU, type_uint,
                builder.createBinOp(spv::Op::OpFAdd, type_float,
                                    builder.createBinOp(spv::Op::OpFMul, type_float, source_color[i][0],
                                                        unorm_scale_rgb),
                                    unorm_round_offset));
            for (uint32_t j = 1; j < 4; ++j) {
              packed[i] = builder.createQuadOp(
                  ...
                  builder.createUnaryOp(
                      ...
                      builder.createBinOp(
                          spv::Op::OpFAdd, type_float,
                          builder.createBinOp(spv::Op::OpFMul, type_float, source_color[i][j],
        ...
        case xenos::ColorRenderTargetFormat::k_2_10_10_10_FLOAT_AS_16_16_16_16: {
          ...
          for (uint32_t i = 0; i < 2; ++i) {
            ...
            for (uint32_t j = 1; j < 3; ++j) {
              packed[i] =
                  builder.createQuadOp(spv::Op::OpBitFieldInsert, type_uint, packed[i],
        ...
        case xenos::ColorRenderTargetFormat::k_32_FLOAT: {
          for (uint32_t i = 0; i < 2; ++i) {
            ...
            if (!source_color_is_uint) {
              packed[i] = builder.createUnaryOp(spv::Op::OpBitcast, type_uint, packed[i]);
            }
          }
        } break;
        case xenos::ColorRenderTargetFormat::k_32_32_FLOAT: {
      ...
      for (uint32_t i = 0; i < 2; ++i) {
        ...
        switch (source_depth_format) {
          case xenos::DepthRenderTargetFormat::kD24S8: {
            ...
            depth24 = builder.createUnaryOp(
                ...
                builder.createUnaryBuiltinCall(
                    type_float, ext_inst_glsl_std_450, GLSLstd450RoundEven,
                    builder.createBinOp(spv::Op::OpFMul, type_float, source_depth_float[i],
        ...
        packed[i] = builder.createQuadOp(spv::Op::OpBitFieldInsert, type_uint, source_stencil[i],
      ...
      if (dest_color_format == xenos::ColorRenderTargetFormat::k_32_32_FLOAT) {
        ...
        if (!dest_color_is_uint) {
          for (spv::Id& float32 : id_vector_temp) {
            float32 = builder.createUnaryOp(spv::Op::OpBitcast, type_float, float32);
          }
        }
    ...
    if (source_is_color) {
      switch (source_color_format) {
        ...
        case xenos::ColorRenderTargetFormat::k_8_8_8_8_GAMMA: {
          if (mode.output == TransferOutput::kStencilBit) {
            ...
            packed = builder.createUnaryOp(
                spv::Op::OpConvertFToU, type_uint,
                builder.createBinOp(spv::Op::OpFAdd, type_float,
                                    builder.createBinOp(spv::Op::OpFMul, type_float, source_color[0][0],
            ...
            packed = builder.createUnaryOp(
                spv::Op::OpConvertFToU, type_uint,
                builder.createBinOp(spv::Op::OpFAdd, type_float,
                                    builder.createBinOp(spv::Op::OpFMul, type_float, source_color[0][1],
                                                        builder.makeFloatConstant(255.0f)),
                                    builder.makeFloatConstant(0.5f)));
            spv::Id component_width = builder.makeUintConstant(8);
            for (uint32_t i = 2; i < 4; ++i) {
              packed = builder.createQuadOp(
                  ...
                  builder.createUnaryOp(
                      ...
                      builder.createBinOp(
                          spv::Op::OpFAdd, type_float,
                          builder.createBinOp(spv::Op::OpFMul, type_float, source_color[0][i],
                                              builder.makeFloatConstant(255.0f)),
                          builder.makeFloatConstant(0.5f))),
          ...
          if (dest_is_color &&
            ...
            if (mode.output != TransferOutput::kStencilBit) {
              ...
              for (uint32_t i = 1; i < 3; ++i) {
                packed =
                    builder.createQuadOp(spv::Op::OpBitFieldInsert, type_uint, packed,
                                         SpirvShaderTranslator::UnclampedFloat32To7e3(
              ...
              packed = builder.createQuadOp(
                  ...
                  builder.createUnaryOp(
                      ...
                      builder.createBinOp(
                          spv::Op::OpFAdd, type_float,
                          builder.createBinOp(spv::Op::OpFMul, type_float, alpha_saturated,
        ...
        case xenos::ColorRenderTargetFormat::k_16_16_16_16_FLOAT: {
          ...
          if (mode.output != TransferOutput::kStencilBit) {
            packed = builder.createQuadOp(spv::Op::OpBitFieldInsert, type_uint, packed,
        ...
        case xenos::ColorRenderTargetFormat::k_32_32_FLOAT: {
          ...
          if (!source_color_is_uint) {
            packed = builder.createUnaryOp(spv::Op::OpBitcast, type_uint, packed);
          }
        } break;
      }
      ...
      if (mode.output == TransferOutput::kDepth && dest_depth_format == source_depth_format) {
        ...
        switch (source_depth_format) {
          case xenos::DepthRenderTargetFormat::kD24S8: {
            ...
            packed = builder.createUnaryOp(
                ...
                builder.createUnaryBuiltinCall(
                    type_float, ext_inst_glsl_std_450, GLSLstd450RoundEven,
                    builder.createBinOp(spv::Op::OpFMul, type_float, source_depth_float[0],
                                        builder.makeFloatConstant(float(0xFFFFFF)))));
          } break;
          case xenos::DepthRenderTargetFormat::kD24FS8: {
        ...
        if (mode.output == TransferOutput::kDepth) {
          ...
          // Merge depth and stencil.
          packed = builder.createQuadOp(spv::Op::OpBitFieldInsert, type_uint, source_stencil[0], packed,
        ...
        if (packed != spv::NoResult) {
          switch (dest_color_format) {
              ...
              for (uint32_t i = 0; i < 4; ++i) {
                id_vector_temp.push_back(builder.createBinOp(
                    ...
                    builder.createUnaryOp(
                        ...
                        builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, packed,
                                            builder.makeUintConstant(8 * i), component_width)),
                    unorm_scale));
              }
            ...
            case xenos::ColorRenderTargetFormat::k_2_10_10_10_FLOAT_AS_16_16_16_16: {
              ...
              id_vector_temp.push_back(builder.createBinOp(
                  spv::Op::OpFMul, type_float,
                  builder.createUnaryOp(spv::Op::OpConvertUToF, type_float,
      ...
      case TransferOutput::kDepth: {
        if (packed) {
          ...
          if (!packed_only_depth) {
            ...
            guest_depth24 = builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, guest_depth24,
          ...
          if (host_depth_source_texture != spv::NoResult) {
              ...
              if (key.host_depth_source_msaa_samples >= xenos::MsaaSamples::k4X) {
                ...
                if (key.dest_msaa_samples == xenos::MsaaSamples::k2X) {
                  ...
                  host_depth_source_tile_pixel_x =
                      builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, dest_tile_pixel_x,
                                          builder.makeUintConstant(1));
                } else {
                  // 4x -> 1x.
                  // Pixels to samples.
                  ...
                  host_depth_source_tile_pixel_y =
                      builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, dest_tile_pixel_y,
                                          builder.makeUintConstant(1));
                }
              } else {
              ...
              if (key.host_depth_source_msaa_samples < xenos::MsaaSamples::k4X) {
                if (key.dest_msaa_samples >= xenos::MsaaSamples::k4X) {
                  ...
                  if (key.host_depth_source_msaa_samples == xenos::MsaaSamples::k2X) {
                    ...
                    if (msaa_2x_attachments_supported_) {
                      host_depth_source_sample_id = builder.createBinOp(
                          spv::Op::OpBitwiseXor, type_uint, host_depth_source_sample_id,
                          builder.makeUintConstant(1));
                    } else {
                      host_depth_source_sample_id = builder.createQuadOp(
                          spv::Op::OpBitFieldInsert, type_uint, host_depth_source_sample_id,
                  ...
                  if (key.host_depth_source_msaa_samples == xenos::MsaaSamples::k2X) {
                    ...
                    host_depth_source_sample_id =
                        builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, dest_tile_pixel_y,
                                            builder.makeUintConstant(1));
                    if (msaa_2x_attachments_supported_) {
                      host_depth_source_sample_id = builder.createBinOp(
                          spv::Op::OpBitwiseXor, type_uint, host_depth_source_sample_id,
            ...
            spv::Id host_depth_address_constant =
                builder.createLoad(builder.createAccessChain(spv::StorageClass::PushConstant,
            ...
            spv::Id host_depth_source_pitch_tiles =
                builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, host_depth_address_constant,
                                    builder.makeUintConstant(xenos::kEdramPitchTilesBits),
                                    builder.makeUintConstant(xenos::kEdramPitchTilesBits));
            spv::Id host_depth_source_tile_index_y =
                builder.createBinOp(spv::Op::OpUDiv, type_uint, host_depth_source_tile_index,
                                    host_depth_source_pitch_tiles);
            spv::Id host_depth_source_tile_index_x =
                builder.createBinOp(spv::Op::OpUMod, type_uint, host_depth_source_tile_index,
                                    host_depth_source_pitch_tiles);
            ...
            if (key.host_depth_source_msaa_samples != xenos::MsaaSamples::k1X) {
              host_depth_source_texture_parameters.sample =
                  builder.createUnaryOp(spv::Op::OpBitcast, type_int, host_depth_source_sample_id);
            } else {
            ...
            id_vector_temp.push_back(
                builder.createUnaryOp(spv::Op::OpBitcast, type_int, host_depth_offset));
            ...
            host_depth32 = builder.createUnaryOp(
                ...
                builder.createLoad(
                    builder.createAccessChain(spv::StorageClass::Uniform, host_depth_source_buffer,
                                              id_vector_temp),
                    spv::NoPrecision));
          }
          ...
          if (host_depth32 != spv::NoResult) {
            ...
            switch (dest_depth_format) {
              case xenos::DepthRenderTargetFormat::kD24S8: {
                ...
                host_depth24 = builder.createUnaryOp(
                    ...
                    builder.createUnaryBuiltinCall(
                        type_float, ext_inst_glsl_std_450, GLSLstd450RoundEven,
                        builder.createBinOp(spv::Op::OpFMul, type_float, host_depth32,
                                            builder.makeFloatConstant(float(0xFFFFFF)))));
              } break;
              case xenos::DepthRenderTargetFormat::kD24FS8: {
            ...
            spv::Id host_depth_outdated =
                builder.createBinOp(spv::Op::OpINotEqual, type_bool, guest_depth24, host_depth24);
            ...
            builder.createSelectionMerge(depth24_to_depth32_merge, spv::SelectionControlMask::MaskNone);
              ...
              guest_depth32 = builder.createBinOp(
                  ...
                  builder.createUnaryOp(
                      ...
                      builder.createBinOp(
                          ...
                          builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, guest_depth24,
          ...
          if (host_depth32 != spv::NoResult) {
            ...
            fragment_depth32 = builder.createOp(spv::Op::OpPhi, type_float, id_vector_temp);
          }
        ...
        if (packed) {
          ...
          SpirvBuilder::IfBuilder stencil_kill_if(
              builder.createBinOp(
                  ...
                  builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, packed, stencil_mask_constant),
                  builder.makeUintConstant(0)),
  ...
  if (output_fragment_stencil_ref != spv::NoResult) {
    builder.addExecutionMode(main_function, spv::ExecutionMode::StencilRefReplacingEXT);
  }
  spv::Instruction* entry_point =
      builder.addEntryPoint(spv::ExecutionModel::Fragment, main_function, "main");
...
VkPipeline VulkanRenderTargetCache::GetDumpPipeline(DumpPipelineKey key) {
  ...
  // Uniform.
  spv::Id edram_buffer =
      builder.createVariable(spv::NoPrecision, spv::StorageClass::Uniform, type_edram, "xe_edram");
  builder.addDecoration(edram_buffer, spv::Decoration::DescriptorSet, kDumpDescriptorSetEdram);
  ...
  spv::Id source_texture = builder.createVariable(
      spv::NoPrecision, spv::StorageClass::UniformConstant,
      builder.makeImageType(source_component_type, spv::Dim::Dim2D, false, false, source_is_multisampled,
                            1, spv::ImageFormat::Unknown),
      "xe_edram_dump_source");
  builder.addDecoration(source_texture, spv::Decoration::DescriptorSet, kDumpDescriptorSetSource);
  ...
  builder.addMemberDecoration(type_push_constants, kDumpPushConstantPitches, spv::Decoration::Offset,
                              int(sizeof(uint32_t) * kDumpPushConstantPitches));
  builder.addMemberName(type_push_constants, kDumpPushConstantOffsets, "offsets");
  builder.addMemberDecoration(type_push_constants, kDumpPushConstantOffsets, spv::Decoration::Offset,
                              int(sizeof(uint32_t) * kDumpPushConstantOffsets));
  builder.addDecoration(type_push_constants, spv::Decoration::Block);
  spv::Id push_constants =
      builder.createVariable(spv::NoPrecision, spv::StorageClass::PushConstant, type_push_constants,
  ...
  spv::Id rectangle_tile_index_x =
      builder.createBinOp(spv::Op::OpUDiv, type_uint, rectangle_sample_x, const_tile_width);
  spv::Id tile_sample_x =
      builder.createBinOp(spv::Op::OpUMod, type_uint, rectangle_sample_x, const_tile_width);
  ...
  spv::Id rectangle_tile_index_y =
      builder.createBinOp(spv::Op::OpUDiv, type_uint, rectangle_sample_y, const_tile_height);
  spv::Id tile_sample_y =
      builder.createBinOp(spv::Op::OpUMod, type_uint, rectangle_sample_y, const_tile_height);
  ...
  spv::Id rectangle_tile_index = builder.createBinOp(
      spv::Op::OpIAdd, type_uint,
      builder.createBinOp(spv::Op::OpIMul, type_uint,
                          builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, pitches_constant,
  ...
  spv::Id edram_tile_index_non_wrapped =
      builder.createBinOp(spv::Op::OpIAdd, type_uint,
                          builder.createTriOp(spv::Op::OpBitFieldUExtract, type_uint, offsets_constant,
  ...
  spv::Id edram_sample_address = builder.createBinOp(
      spv::Op::OpIAdd, type_uint,
      builder.createBinOp(
          spv::Op::OpIMul, type_uint, builder.makeUintConstant(tile_width * tile_height),
          builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, edram_tile_index_non_wrapped,
                              builder.makeUintConstant(xenos::kEdramTileCount - 1))),
      builder.createBinOp(
          spv::Op::OpIAdd, type_uint,
          builder.createBinOp(spv::Op::OpIMul, type_uint, const_tile_width, tile_sample_y),
          tile_sample_x));
  if (key.is_depth) {
    ...
    edram_sample_address = builder.createUnaryOp(
        ...
        builder.createBinOp(
            spv::Op::OpIAdd, type_int,
            builder.createUnaryOp(spv::Op::OpBitcast, type_int, edram_sample_address),
            builder.createTriOp(
                spv::Op::OpSelect, type_int,
                builder.createBinOp(spv::Op::OpULessThan, builder.makeBoolType(), tile_sample_x,
  ...
  spv::Id source_sample_x = builder.createBinOp(
      spv::Op::OpIAdd, type_uint,
      builder.createBinOp(spv::Op::OpIMul, type_uint, const_tile_width, source_tile_index_x),
      tile_sample_x);
  spv::Id source_sample_y = builder.createBinOp(
      spv::Op::OpIAdd, type_uint,
      builder.createBinOp(spv::Op::OpIMul, type_uint, const_tile_height, source_tile_index_y),
      tile_sample_y);
  ...
  if (source_is_multisampled) {
    ...
    source_pixel_y =
        builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, source_sample_y, const_uint_1);
    if (key.msaa_samples >= xenos::MsaaSamples::k4X) {
      source_pixel_x =
          builder.createBinOp(spv::Op::OpShiftRightLogical, type_uint, source_sample_x, const_uint_1);
      // 4x MSAA source texture sample index - bit 0 for horizontal, bit 1 for
      // vertical.
      source_sample_id = builder.createQuadOp(
          spv::Op::OpBitFieldInsert, type_uint,
          builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, source_sample_x, const_uint_1),
      ...
      source_sample_id = builder.createTriOp(
          ...
          builder.createBinOp(
              ...
              builder.createBinOp(spv::Op::OpBitwiseAnd, type_uint, source_sample_y, const_uint_1),
              const_uint_0),
  ...
  id_vector_temp.clear();
  id_vector_temp.push_back(builder.createUnaryOp(spv::Op::OpBitcast, type_int, source_pixel_x));
  id_vector_temp.push_back(builder.createUnaryOp(spv::Op::OpBitcast, type_int, source_pixel_y));
  ...
  auto LinearToPWLGamma = [&](spv::Id linear, bool linear_pre_saturated) -> spv::Id {
    ...
    spv::Id is_piece_at_least_3 =
        builder.createBinOp(spv::Op::OpFOrdGreaterThanEqual, builder.makeBoolType(), linear,
                            builder.makeFloatConstant(512.0f / 1023.0f));
    spv::Id scale_3_or_2 = builder.createTriOp(spv::Op::OpSelect, type_float, is_piece_at_least_3,
                                               builder.makeFloatConstant(1023.0f / 8.0f),
                                               builder.makeFloatConstant(1023.0f / 4.0f));
  ...
  auto PackDumpSource16PairToUint32 = [&](spv::Id component_0, spv::Id component_1) -> spv::Id {
    return builder.createQuadOp(
        spv::Op::OpBitFieldInsert, type_uint, PackDumpSource16ComponentToUint(component_0),
  ...
  if (key.is_depth) {
    ...
    switch (key.GetDepthFormat()) {
      case xenos::DepthRenderTargetFormat::kD24S8: {
        ...
        packed[0] = builder.createUnaryOp(
            ...
            builder.createUnaryBuiltinCall(
                type_float, ext_inst_glsl_std_450, GLSLstd450RoundEven,
                builder.createBinOp(spv::Op::OpFMul, type_float, source_depth32,
                                    builder.makeFloatConstant(float(0xFFFFFF)))));
      } break;
      case xenos::DepthRenderTargetFormat::kD24FS8: {
    ...
    packed[0] = builder.createQuadOp(spv::Op::OpBitFieldInsert, type_uint, source_stencil, packed[0],
                                     builder.makeUintConstant(8), builder.makeUintConstant(24));
  } else {
    switch (key.GetColorFormat()) {
      ...
      case xenos::ColorRenderTargetFormat::k_8_8_8_8: {
        ...
        packed[0] = builder.createUnaryOp(
            spv::Op::OpConvertFToU, type_uint,
            builder.createBinOp(spv::Op::OpFAdd, type_float,
                                builder.createBinOp(spv::Op::OpFMul, type_float, color_0, unorm_scale),
                                unorm_round_offset));
        spv::Id component_width = builder.makeUintConstant(8);
        for (uint32_t i = 1; i < 4; ++i) {
          ...
          packed[0] = builder.createQuadOp(
              ...
              builder.createUnaryOp(spv::Op::OpConvertFToU, type_uint,
                                    builder.createBinOp(spv::Op::OpFAdd, type_float,
                                                        builder.createBinOp(spv::Op::OpFMul, type_float,
                                                                            color_i, unorm_scale),
                                                        unorm_round_offset)),
      ...
      case xenos::ColorRenderTargetFormat::k_32_32_FLOAT: {
        for (uint32_t i = 0; i <= uint32_t(format_is_64bpp); ++i) {
          ...
          if (!source_is_uint) {
            packed_ref = builder.createUnaryOp(spv::Op::OpBitcast, type_uint, packed_ref);
          }
        }
      } break;
    }
  ...
  // The only SSBO structure member.
  id_vector_temp.push_back(builder.makeIntConstant(0));
  id_vector_temp.push_back(builder.createUnaryOp(spv::Op::OpBitcast, type_int, edram_sample_address));
  // StorageBuffer since SPIR-V 1.3, but since SPIR-V 1.0 is generated, it's
  // Uniform.
  builder.createStore(store_value, builder.createAccessChain(spv::StorageClass::Uniform, edram_buffer,
                                                             id_vector_temp));

  // End the main function and make it the entry point.
  builder.leaveFunction();
  builder.addExecutionMode(main_function, spv::ExecutionMode::LocalSize, kDumpSamplesPerGroupX,
