...
SpirvShaderTranslator::Modification VulkanPipelineCache::GetCurrentPixelShaderModification(
  ...
  // Vulkan fixed-function blend ignores factors for MIN/MAX, but Xbox 360
...
VkShaderModule VulkanPipelineCache::GetGeometryShader(GeometryShaderKey key) {
  ...
  builder.addDecoration(type_float4_array_6, spv::Decoration::ArrayStride, sizeof(float) * 4);
  ...
  if (key.type == PipelineGeometryShader::kPointList) {
    ...
    builder.addMemberDecoration(
        type_system_constants, kPointConstantConstantDiameter, spv::Decoration::Offset,
    ...
    builder.addMemberDecoration(
        type_system_constants, kPointConstantScreenDiameterToNdcRadius, spv::Decoration::Offset,
    ...
    builder.addMemberDecoration(
        type_system_constants, kPointConstantUserClipPlanes, spv::Decoration::Offset,
  ...
  if (type_system_constants != spv::NoType) {
    ...
    uniform_system_constants =
        builder.createVariable(spv::NoPrecision, spv::StorageClass::Uniform, type_system_constants,
                               "xe_uniform_system_constants");
    builder.addDecoration(uniform_system_constants, spv::Decoration::DescriptorSet,
  ...
  spv::Id in_gl_per_vertex = builder.createVariable(spv::NoPrecision, spv::StorageClass::Input,
  ...
  for (uint32_t i = 0; i < key.interpolator_count; ++i) {
    spv::Id out_interpolator =
        builder.createVariable(spv::NoPrecision, spv::StorageClass::Output, type_float4,
                               fmt::format("xe_out_interpolator_{}", i).c_str());
    out_interpolators[i] = out_interpolator;
    builder.addDecoration(out_interpolator, spv::Decoration::Location, int(output_location));
  ...
  if (key.has_point_coordinates) {
    out_point_coordinates = builder.createVariable(spv::NoPrecision, spv::StorageClass::Output,
                                                   type_float2, "xe_out_point_coordinates");
    builder.addDecoration(out_point_coordinates, spv::Decoration::Location, int(output_location));
  ...
  for (uint32_t i = 0; i < key.interpolator_count; ++i) {
    ...
    builder.addDecoration(in_interpolator, spv::Decoration::Location, int(input_location));
  ...
  if (key.has_point_size) {
    in_point_size = builder.createVariable(
        ...
        "xe_in_point_size");
    builder.addDecoration(in_point_size, spv::Decoration::Location, int(input_location));
  ...
  if (clip_distance_count) {
    ...
    builder.addMemberDecoration(type_struct_out_gl_per_vertex,
                                member_out_gl_per_vertex_clip_distance, spv::Decoration::BuiltIn,
                                static_cast<int>(spv::BuiltIn::ClipDistance));
  }
  builder.addDecoration(type_struct_out_gl_per_vertex, spv::Decoration::Block);
  spv::Id out_gl_per_vertex = builder.createVariable(spv::NoPrecision, spv::StorageClass::Output,
  ...
  spv::Instruction* entry_point =
      builder.addEntryPoint(spv::ExecutionModel::Geometry, main_function, "main");
  ...
  if (denorm_flush_to_zero_float32) {
    builder.addCapability(spv::Capability::DenormFlushToZero);
    builder.addExecutionMode(main_function, spv::ExecutionMode::DenormFlushToZero, 32);
  }
  if (signed_zero_inf_nan_preserve_float32) {
    builder.addCapability(spv::Capability::SignedZeroInfNanPreserve);
    builder.addExecutionMode(main_function, spv::ExecutionMode::SignedZeroInfNanPreserve, 32);
  }
  if (rounding_mode_rte_float32) {
    builder.addCapability(spv::Capability::RoundingModeRTE);
    builder.addExecutionMode(main_function, spv::ExecutionMode::RoundingModeRTE, 32);
  }
  ...
  for (uint32_t i = 0; i < input_primitive_vertex_count; ++i) {
    ...
    spv::Id position_is_nan = builder.createUnaryOp(
        ...
        builder.createUnaryOp(
            ...
            builder.createLoad(
                builder.createAccessChain(spv::StorageClass::Input, in_gl_per_vertex, id_vector_temp),
                spv::NoPrecision)));
    ...
    builder.createSelectionMerge(&discard_merge_block, spv::SelectionControlMask::DontFlatten);
    {
      ...
      for (uint32_t i = cull_distance_start; i < cull_distance_count; ++i) {
        for (uint32_t j = 0; j < input_primitive_vertex_count; ++j) {
          ...
          spv::Id cull_distance_is_negative = builder.createBinOp(
              ...
              builder.createLoad(builder.createAccessChain(spv::StorageClass::Input, in_gl_per_vertex,
                                                           id_vector_temp),
                                 spv::NoPrecision),
              const_float_0);
          if (cull_condition != spv::NoResult) {
            cull_condition = builder.createBinOp(spv::Op::OpLogicalAnd, type_bool, cull_condition,
  ...
  switch (key.type) {
    case PipelineGeometryShader::kPointList: {
      ...
      spv::Id point_guest_diameter_x =
          builder.createLoad(builder.createAccessChain(spv::StorageClass::Uniform,
      ...
      if (key.has_point_size) {
        ...
        spv::Id point_vertex_diameter = builder.createLoad(
            builder.createAccessChain(spv::StorageClass::Input, in_point_size, id_vector_temp),
            spv::NoPrecision);
        spv::Id point_vertex_diameter_written = builder.createBinOp(
            spv::Op::OpFOrdGreaterThanEqual, type_bool, point_vertex_diameter, const_float_0);
        point_guest_diameter_x =
            builder.createTriOp(spv::Op::OpSelect, type_float, point_vertex_diameter_written,
      ...
      point_radius_x =
          builder.createNoContractionBinOp(spv::Op::OpFMul, type_float, point_radius_x, point_w);
      point_radius_y =
          builder.createNoContractionBinOp(spv::Op::OpFMul, type_float, point_radius_y, point_w);
      ...
      for (uint32_t i = 0; i < key.interpolator_count; ++i) {
        point_interpolators[i] = builder.createLoad(
            builder.createAccessChain(spv::StorageClass::Input, in_interpolators[i], id_vector_temp),
            spv::NoPrecision);
      }
      // Positions.
      ...
      if (point_recalculate_clip_distances || point_recalculate_cull_distances) {
        ...
        for (uint32_t i = 0; i < point_user_clip_plane_count; ++i) {
          ...
          point_user_clip_planes.push_back(builder.createLoad(
              builder.createAccessChain(spv::StorageClass::Uniform, uniform_system_constants,
                                        id_vector_temp),
              spv::NoPrecision));
        }
      ...
      if (point_recalculate_cull_distances) {
        ...
        for (uint32_t i = 0; i < 4; ++i) {
          ...
          spv::Id point_guest_position_xyz = builder.createNoContractionBinOp(
              spv::Op::OpFSub, type_float3, point_vertex_position_xyz,
              builder.createNoContractionBinOp(spv::Op::OpVectorTimesScalar, type_float3,
                                               point_ndc_offset, point_w));
          point_guest_position_xyz = builder.createNoContractionBinOp(
              spv::Op::OpFDiv, type_float3, point_guest_position_xyz, point_ndc_scale);
          id_vector_temp.clear();
          id_vector_temp.push_back(
        ...
        for (uint32_t i = 0; i < point_user_cull_distance_count; ++i) {
          ...
          for (uint32_t j = 0; j < 4; ++j) {
            spv::Id point_distance_is_negative = builder.createBinOp(
                spv::Op::OpFOrdLessThan, type_bool,
                builder.createBinOp(spv::Op::OpDot, type_float, point_guest_positions[j],
                                    point_user_clip_planes[i]),
                const_float_0);
            if (point_plane_cull_condition != spv::NoResult) {
              point_plane_cull_condition =
                  builder.createBinOp(spv::Op::OpLogicalAnd, type_bool, point_plane_cull_condition,
          ...
          if (point_cull_condition != spv::NoResult) {
            point_cull_condition = builder.createBinOp(
                spv::Op::OpLogicalOr, type_bool, point_cull_condition, point_plane_cull_condition);
          } else {
        ...
        builder.createSelectionMerge(&point_cull_merge_block, spv::SelectionControlMask::DontFlatten);
        {
          std::unique_ptr<spv::Instruction> branch_conditional_op(
              std::make_unique<spv::Instruction>(spv::Op::OpBranchConditional));
      ...
      for (uint32_t i = 0; i < 4; ++i) {
        ...
        builder.createStore(
            ...
            builder.createAccessChain(spv::StorageClass::Output, out_gl_per_vertex, id_vector_temp));
        // Clip distances.
        if (point_recalculate_clip_distances) {
          ...
          for (uint32_t j = 0; j < clip_distance_count; ++j) {
            ...
            id_vector_temp.push_back(builder.makeIntConstant(int32_t(j)));
            builder.createStore(builder.createBinOp(spv::Op::OpDot, type_float, point_guest_position,
          ...
          builder.createStore(point_clip_distances,
                              builder.createAccessChain(spv::StorageClass::Output, out_gl_per_vertex,
                                                        id_vector_temp));
        }
        // Emit the vertex.
    ...
    case PipelineGeometryShader::kRectangleList: {
      ...
      for (uint32_t i = 0; i < 3; ++i) {
        ...
        spv::Id edge_1_y = builder.createLoad(
            builder.createAccessChain(spv::StorageClass::Input, in_gl_per_vertex, id_vector_temp),
            spv::NoPrecision);
        spv::Id edge_x = builder.createBinOp(spv::Op::OpFSub, type_float, edge_1_x, edge_0_x);
        spv::Id edge_y = builder.createBinOp(spv::Op::OpFSub, type_float, edge_1_y, edge_0_y);
        edge_lengths[i] = builder.createBinOp(
            spv::Op::OpFAdd, type_float, builder.createBinOp(spv::Op::OpFMul, type_float, edge_x, edge_x),
      ...
      vertex_indices[0] = builder.createTriOp(
          ...
          const_int_0,
          builder.createTriOp(spv::Op::OpSelect, type_int,
                              builder.createBinOp(spv::Op::OpFOrdGreaterThan, type_bool,
      ...
      for (uint32_t i = 1; i < 3; ++i) {
        ...
        spv::Id vertex_index_without_wrapping = builder.createBinOp(
            spv::Op::OpIAdd, type_int, vertex_indices[0], builder.makeIntConstant(int32_t(i)));
        vertex_indices[i] = builder.createTriOp(
            spv::Op::OpSelect, type_int,
            builder.createBinOp(spv::Op::OpSLessThan, type_bool, vertex_index_without_wrapping,
                                const_int_3),
            vertex_index_without_wrapping,
            builder.createBinOp(spv::Op::OpISub, type_int, vertex_index_without_wrapping, const_int_3));
      }
      ...
      for (uint32_t i = 0; i < 3; ++i) {
        ...
        for (uint32_t j = 0; j < key.interpolator_count; ++j) {
          builder.createStore(
              builder.createLoad(builder.createAccessChain(spv::StorageClass::Input,
      ...
      for (uint32_t i = 0; i < key.interpolator_count; ++i) {
        ...
        spv::Id vertex_interpolator_v0 = builder.createLoad(
            builder.createAccessChain(spv::StorageClass::Input, in_interpolator, id_vector_temp),
            spv::NoPrecision);
