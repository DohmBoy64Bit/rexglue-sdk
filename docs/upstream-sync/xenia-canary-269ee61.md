...
| 2026-06-19 | Vulkan SPIRV enum migration | `vulkan/render_target_cache.cpp` | — | Fixed remaining old-style spv:: enum references (BuiltIn casts, makeFunctionEntry LinkageType, double-wrapped createBranch, ExecutionModel corruption) — caught during full Vulkan build | ✅ Complete |
| 2026-06-19 | spirv-tools build fix | `thirdparty/CMakeLists.txt` | — | Added `-w` suppression for all spirv-tools targets (SPIRV-Tools-static/opt/reduce/diff/link/lint/fuzz/shared) — resolves clang-cl `-Werror` clash with global `-ffp-model=strict`/`-fno-char8_t` flags | ✅ Complete |
| 2026-06-19 | mspack symlink fix | `thirdparty/libmspack/cabextract/mspack/*` | — | Copied real source files over broken symlink redirect files (14 .c/.h files) — Windows git doesn't preserve symlinks | ✅ Complete |
